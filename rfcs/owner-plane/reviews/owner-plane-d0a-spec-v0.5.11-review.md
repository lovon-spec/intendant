# Review: D0-A Core + Memory normative specification v0.5.11

Reviewed source: `~/owner-plane-d0a-spec.md`, v0.5.11 freeze candidate,
SHA-256
`e304a5d9e8669a6fcfbdd2a73534d939b2bb5dff7ca838516281cef5d28815f1`.
The archived v0.5.10 input matches the source used for the previous review
(`8b3e069795978808e0c49e2cb341e784843e8bff9b4232328d1104834ce39ddc`).

## Executive verdict

Do not freeze v0.5.11. Cut v0.5.12.

This is a productive revision. D-135 fixes the first-ratification identity bug
and makes requester snapshots total. D-138 replaces the unportable recovery
“past base” rule with a state an independent replica can actually fold. D-141
corrects the false retention bound, makes cap existence derived, makes KEM
custody transitive, and extends signing-key freshness to enrollments. D-142
provides a constructible `mimport` schema and the missing equality matrix.
Those are substantive closures, not wording improvements.

The freeze claim nevertheless fails under concrete traces. The most important
problem is D-136: one lexicographic Head cannot both preserve several live
generation heads and close every extension, while folding that Head into an
authority-agnostic `immutable_cap` also blocks the legitimate successor
authority. D-137 reserves only one coordinate rather than the unresolved
fork-selection scope. D-139
does not establish a lifetime import lineage or stable claim identity. D-140's
reopen path lacks deterministic basis selection and terminal-effect
reincarnation or reconciliation semantics. A separate pre-existing transfer
problem is now unavoidable: because the flat bundle is never persisted, a
committed partial import cannot be validated from portable bytes after another
source record is erased.

The key lifecycle also needs one more recovery pass. Incorporation-cap
eligibility is self-referential; KEM public keys may be reused across devices;
and C3′ can remove a renewal after the old KEM secret has been irreversibly
destroyed. Finally, the fork-inclusive “ever enrolled” signing-key set has no
portable recovery commitment, so replicas can disagree about whether a key is
fresh.

This is still a repair round, not an architectural rejection. The Owner Plane,
Memory claim model, control/tenant separation, and claims-not-facts posture do
not need reopening. The remaining work is to finish the exact closure,
identity, proof, and recovery machinery already selected.

## Adjudicated disposition ledger

| Prior issue or new decision | v0.5.11 disposition | Assessment |
|---|---|---|
| First ratification was `max(Top, H)`; empty snapshot retired nothing | D-135 tagged `Unbounded / Bounded` state and total snapshot override | **Fixed** |
| Authority ending under D-130 | D-136 replaces promotion with scalar close/supersede boundaries | **Open — blocker** |
| Pending tenant selector ordering | D-137 reserves the exact Head coordinate | **Partial — blocker** |
| Recovery omission had no tenant coordinate | D-138 implicit revivable `"none"` override | **Partial**: portable mechanism fixed; complement domain unresolved |
| C3′ removal symmetry | D-138 mandates a total fresh re-fold | **Partial**: disposition and terminal lifecycles still contradict it |
| Space retirement | Epoch event, CDDL body, and mandatory close coverage added | **Partial**: its backdate defense inherits D-136 |
| Import claim identity | D-139 permits one active import grant per zone | **Open — blocker** |
| Transfer terminal stability | D-140 adds `basis` and `XferReopen` | **Partial — blocker** |
| Cap authority and retention | D-141 ratifies the receipt exception, derives eligibility, and corrects retention | **Partial**: cap eligibility remains circular |
| Renewal KEM and signing-key closure | Transitive KEM custody and plane-wide signing-key freshness | **Partial**: recovery portability and KEM reuse remain open |
| Impossible `mimport` plus missing mirrors | D-142 narrow schema and equality matrix | **Fixed at the schema level** |
| Durable verification of a partial import after source erasure | Not addressed | **New blocker** |
| Gate-A companion and corpus | Still declared artifact-pending | **Gate remains open** |
...
The repair needs an authority-scoped frozen Frontier, not a scalar Head:
...
The unknown-gap cap makes the legal per-lineage Frontier bounded; its exact
representation still has to include every live generation head. Wide-zone
staging can be paged, but it needs a final commitment that names the consumer
and freezes the complete Frontier.
...
record, say so directly; that is a different irreversible policy from D-140's
stated re-derivation posture. Distinguish intrinsic rejection from
boundary-derived rejection and re-evaluate whenever any predicate necessary to
the effective terminal changes.
...
current and future generation of a covered pair inherits the implicit override.
This is fold-affecting domain semantics, not merely editorial precision.
...
3. **Remove the dead import arm from `mclaim`.** D-142 correctly requires
   `m.propose`/`m.assert` to reject `provenance.import`, and actual imports use
   `mimport`; keeping the optional arm in `mclaim` leaves dead shared vocabulary
   whose non-conformance depends on a prose invariant
   ([CDDL](/Users/vm/owner-plane-d0a-spec.md:3635)).
4. **Complete the D-142 mirror battery.** Its family-11 clause currently names
...
1. Replace scalar authority close with an authority-scoped frozen Frontier and
   update epoch advance, renewal, revocation, recovery, and space retirement
   together.
...
3. Generalize pending tenant selector reservation to the unresolved selection
   scope.
4. Choose logical imported-claim identity and add durable per-record release
   proofs.
...
8. Close the renewal historical-authorship ambiguity and exactness definitions.
...
It is not a candidate baseline yet. The remaining counterexamples can cause
