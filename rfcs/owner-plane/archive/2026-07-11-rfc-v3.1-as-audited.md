...
- **Degraded lane**: hosted-tab genesis survives solely for the true
  ...
  - at the **first trusted-device enrollment**, the product aggressively
    nudges the **hosted-to-trusted re-root ceremony** — §3.1's succession
    machinery paying for itself. One narrow flow: (1) the signed client
    locally verifies its distribution identity to the extent the platform
    permits; (2) recovery material is entered only on that trusted client;
    (3) it independently fetches and verifies the complete control history
    and pinned genesis state; (4) it generates a **fresh native-held
    administrative root and a fresh independent recovery authority**
    (recovering only the hosted root would improve custody without
    resolving compromise — §3.1); (5) it submits the succession proof,
    then independently fetches and displays the accepted post-succession
    state; (6) hosted-root authority is permanently reduced to the
    declared hosted ceiling;
...
- **Accepted consequence**: platform developer signing — the previously
  ...
  **consumer default's critical path**. GPG + the existing
  ...
  self-hosted lane (Linux consumer entry included). Delivery status per
  artifact: the release-transparency log and signed release manifests are
  **live in production today**; the notarized macOS/iOS client, the
  Authenticode Windows client, and zero-daemon native-client packaging
  are **product deliverables to build** — the current macOS wrapper is
  not yet that client.
...
reveals operation content. **D0-A freezes the local layers**: item
ciphertext identity, item key-envelope identity, tombstone/erase-marker
format, and `body_hash` semantics — `body_hash` covers the **canonical
plaintext operation body** (it sits inside the signature and can never be
re-decided); ciphertext content addressing is a separate, outer identity.
**D0-B freezes the distribution layers** — the segment container and the
exact public manifest schema **with its leakage documented**: Connect need
not see real signing-key IDs (it indexes opaque feed handles), but
incremental sync inevitably exposes stable routing handles, segment
order/ranges, sizes, timing, **recipient-set cardinality, and
membership-change events**. D0-B also specifies: AEAD choice, nonce
construction, AAD binding (manifest ↔ container ↔ item records),
compression and padding policy, partial-upload commit, and recipient-wrap
binding.
...
- `certificate_hash` + `capability_hash` are the **DeviceCapability arm**
  of §3.1's `AuthorizationProof`; control operations (genesis, succession,
  recovery) carry the corresponding other arms instead — the envelope's
  authorization slot is the tagged proof, so bootstrap is representable
  without circularity.
...
- **one frozen logical frontier map** (D0-B): every surface that says
  "frontier" means the same per-writer-heads structure;
...
- v1 recipient wraps are **HPKE (RFC 9180) base mode with
  DHKEM(P-256, HKDF-SHA256), HKDF-SHA256, and AES-256-GCM** — the suite
  matching the only shipped custody precedent (the vault's P-256 ECDH →
  HKDF-SHA256 → AES-256-GCM construction; browser identity, vault
  custody, and Web Push are all P-256 today, and no X25519 custody path
  exists in source), with universal WebCrypto support;
...
Judgment (append-only): Accept | Dispute | Supersede | Retract | Retire {
  target_claim (causal ref), actor, capability_hash,
  policy_id + policy_version, reason / evidence_refs
}

MemoryPin / MemoryUnpin (append-only, owner-curated) {
...
hashes**; short-lived per-session taint refusal is defense in depth with
its scope stated honestly (no promise of perfect dynamic taint tracking
across sessions or colluding principals). Either way, separate calls
cannot bypass export policy.
...
- **Channel 1 — `intendant ctl` verbs taught via skills (primary).**
  ...
  **hardening, not invention**: today the derived token is validated
  ...
  local tokens to the intended local transport/caller (default mTLS
  already mitigates many non-loopback requests); make plane grants fail
  closed. **Two lanes, one boundary**: external process trees get this
...
- **Ephemeral by convention, with two hard rules**: mtime-TTL staleness
  ...
  state**: an orchestration checkpoint generation is retained until its
  parent goal/session ends or a successor acknowledges it. It must never
...
- **Live-audio always-consent fix — LANDED (PR #236, merged 2026-07-11)**:
  ...
  ApproveAll approves the single prompt only. **Chokepoint hardening (PR
  #238)**: the gate now mints a single-use, unforgeable `SpawnConsent`
  token and `run_session` requires it — a future dispatch path that skips
  the gate is a compile error, not a policy bug (the review's
  guard-at-the-chokepoint recommendation, implemented by construction).
...
- **D0-A — Core + Memory** (gate for P1 durable data): genesis
  ...
  **the complete local object and key format** — canonical signed
  operation bytes + golden vectors (§4.1), item ciphertext + key-envelope
  identities and `body_hash` semantics (§4.0/§4.7), tombstone/erase-
  marker format, atomic local log/outbox records; typed fail-closed plane
...
- **D0-B — Sync** (gate for P2): segment container + public manifest +
  ...
  design + one frozen frontier map + witness receipts (§4.3, §4.4);
...
- **P2a — storage and first sync**, five explicit sub-deliverables:
  ...
  (§4.2); offline-convergence torture stays P2b. **Zero-daemon exit
  ...
  eviction re-sync; hosted-to-trusted re-root; loss recovery through a
