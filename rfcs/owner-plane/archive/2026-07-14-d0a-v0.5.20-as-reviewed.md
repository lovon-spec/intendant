...
*v0.5.20 draft, 2026-07-14. Supersedes v0.1–v0.5.19
...
- **T5 (lease binding, non-backdateable).** An operation under an
  ...
  timely receipt does not re-open the verdict, and convergence rides
  the writer's re-proposed operation. Held timely evidence beats
  held late evidence at first evaluation. No receipt at all stays
  `lease-missing` (pending); a lease whose window exceeds
...
every order, so the harness's choice is unobservable.
