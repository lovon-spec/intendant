...
*v0.5.13 draft, 2026-07-13. Supersedes v0.1–v0.5.12
...
`~/agenda-rfc-archive/*d0a-v0.*-as-reviewed.md` (v0.1 through v0.5.12).
Folds the successive synthesized reviews (v0.4 through v0.5.12,
`~/owner-plane-d0a-spec-v0.5.12-synthesized-review.md` latest) with the
...
- **T3 (anti-replay and issuer lifecycle).** `issuer_seq` is
  ...
  **Every non-genesis certificate REQUIRES a fresh signing key** —
  ...
  T2/T5). **The freshness domain is portable (D-150/D-158)**: the
  surviving chain's enrollments ∪ the `retired_keys` committed by
  accepted recoveries ∪ the **typed key history of adopted
  renewals** (D-158 — an adopted renewal's signing key is burned
  globally and its KEM key is bound to the adopted device: same-
  device recovery re-enrollment with it is legal, any other device
  rejects; v0.5.12 left adopted keys OUTSIDE the domain, so a later
  ...
  NOT carry in `retired_keys` and does not adopt is re-usable — a
  stated, vectored
...
- **Cross-zone transfer (same daemon)** is two commits in two zone
  ...
  **transition fold** — one EFFECTIVE terminal per interval,
  ...
  its own optional `basis` — **the op hash of the canonical FIRST
  branch-relative fact, in fold order, that made the record
  resolved-negative (D-157)**: a control boundary (seal, void,
  eligible cap, retirement, revocation) OR a tenant fact (the
  winning claimant behind an `import-collision`) — the closed cause
  vocabulary is exactly D-149's branch-relative class; **basis
  ...
  incarnation }`** and recovery re-derives **per record**:
...
continue freely. **Composition is intersection over matching
selectors (D-159)**: an operation admits only within the bounds of
EVERY closure whose selector matches it; the ABSENCE of a closure
for some (authority, zone, lineage) leaves position-relative
validity (D-86) governing — this is the renewal-after-revocation
...
- **C3′ (recovery resolution, deterministic).**
  ...
  **`adopted_renewals` follow the same discipline (D-158)**: each
  ...
  whose renewal does not precede `base`, or a noncontiguous chain
...
- **`w.gen`** (registry §11.1): the charged operation opening
  ...
  retires **exactly the named head** — `last_known` MUST be
  accepted, a valid **chain-membership position** of its generation
  (the D-144 anchor, extended here by D-159 — NEVER terminality:
...
content_digest = the MERKLE ROOT (D-147/D-156):
  leaf_i = H_brec({export_id, rec_index: i,
                   rec: canonical bundlerec bytes})
                             # SELF-DESCRIBING leaves — context and
                             #   ordinal inside every leaf; no
                             #   header leaf exists (bhdr retired)
...
that already escaped is the standing residual. Pre-freeze, an
order-earlier claimant DISPLACES the provisional owner (standard
displacement — the charge
releases, the suffix re-derives, disposition quarantine-reproposal
— revivable if the winner later dies), and judgments, pins, or
erase
requests targeting a provisional import claim are
**pending-dependency** until it freezes; a claim that can never
win
— order-later against a held claim, or any claim against a frozen
owner — is `import-collision` (reject-permanent).
v0.5.10's
...
re-materializes — D-153); a post-advance `w.gen`
...
completed renewal — in both arrival orders — either adopts it
(`adopted_renewals`, signing-only intermediate links included) or
the
K0-destroyed device is `storage-orphaned`, never silently
re-keyed; adopted-key reuse negatives (the adopted KEM key on a
DIFFERENT device's certificate rejects; the adopted signing key
anywhere rejects — D-150/D-158); 9 time/lease (self-receipt
...
winning claimant — is later displaced → reopen (D-157);
...
zoneheads = { zone_id: ulid, heads: [* head] }
  ...
  ;   current heads); the staging lane
...
crecovsucc = { base: { seq: uint, op: bytes32 }, epoch: uint,
  ...
  ? adopted_renewals: [* { device_id: bytes16,
      ...
      ;   its keys join the TYPED freshness history (D-158: signing
      ;   key burned globally; KEM key bound to the adopted device —
      ;   same-device recovery re-enrollment legal, any other
      ;   device rejects) — or
      ...
      ;   renewals (D-158 — dependency closure); wrong-device /
      ;   non-renewal / not-before-base / noncontiguous =
...
xferreopen = { export_id: bytes16, release_op: bytes32,
  incarnation: uint }
                             ; D-140/D-148/D-157: appended when ANY
                             ;   recorded basis invalidates —
...
mimport = { source_op: bytes32,   ; the released bundlerec this binds to
  ...
  proof: [* bytes32],               ; Merkle siblings, <= 7 (E8) —
                                    ...
                                    ;   H_brec({export_id,
                                    ;   rec_index, rec}) folded up
                                    ;   proof (bottom-up, exact
                                    ;   consumption) reaches
                                    ;   release.content_digest,
                                    ;   else body-invariant
                                    ;   (D-147/D-156: per-record
                                    ;   durable validation — binds
                                    ;   the layout, survives source
                                    ;   erasure and rebuild)
  provenance: {
  ...
  ;   matching authority-ending frontier; unfreezes if the basis
  ;   dissolves — an order-losing or anti-frozen claim =
  ;   import-collision
