...
*v0.5.2 draft (freeze candidate), 2026-07-12. Supersedes v0.1–v0.5.1
...
- **E8 (hard caps — deterministic parsing).**
  ...
  | KEK wraps per operation | ≤ 128 (a canonical `kekwrap` ≈ 306 B → ≈ 38 KiB; memberships beyond 128 complete via current-epoch `c.wrap_add` before `RewrapComplete` — D-77) |
  ...
  | rotation `erase_manifest` (typed entries) | ≤ 128 (≈ 14 KiB; joint worst case 128 wraps + 128 entries + header/signature ≈ 55 KiB < 64 KiB — D-77; the erase queue drains across successive rotations, §5.4) |
  | revocation `rotation_refs` | ≤ 64 (continue via `c.revoke_zones`) |
  | revocation `cutoffs` | ≤ 64 (continue via `c.revoke_zones`) |
...
`reauth`, `cutoffreq`, `ckpt`, `survivors`. Derivation domains:
...
- **T3 (anti-replay and issuer lifecycle).** `issuer_seq` is
  ...
  **one counter per scope, shared by receipts and leases alike,
  ...
  durable store. Density is load-bearing (D-77): a compromise cutoff
  at `through = N` leaves no unused sequence ≤ N to backfill — any
  below-cutoff mint collides with an existing statement and
  quarantines as `issuer-fork`; **key renewal restarts the scope at 1** (a new
...
| `c.lineage_reauth` | admin (hosted: own lineage only, §7.5) | opens a new `max_generations` window on the lineage; **budgets and cutoffs persist**; body carries a REQUIRED **requester attestation** `Sign(device_sk, msg("reauth", { plane_id, lineage, max_generations, request_id, ctrl_frontier, window_state }))` — bound to this operation's `request_id` (single-use, D-59) **and to the lineage's current window state** (`window_state` = the count of accepted `w.gen` operations for the lineage since its last reauthorization or creation; admission requires equality — a banked attestation goes stale the moment the window moves, D-72); the admin-arm header alone cannot prove "own lineage" | reject-permanent |
...
| `c.cutoff` | admin (hosted: own lineage only, §7.5) | zone-qualified cutoff `{ zone_id, lineage, accepted_through }`; MAY carry a **requester attestation** `Sign(device_sk, msg("cutoffreq", { plane_id, zone_id, lineage, accepted_through, request_id, ctrl_frontier, window_state }))` — REQUIRED on hosted planes (D-54); `requester.device_cert` resolves to a certificate whose `device_id` owns the target lineage and the signature verifies under that certificate's key (D-77); bound to this operation's `request_id` (single-use, D-59) and the lineage's `window_state` (D-72) | reject-permanent |
| `c.abandon_writer` | admin | lineage abandoned at head | reject-permanent |
| `c.checkpoint` | admin | **zone-scoped (D-62)**: names its `zone_id`; `checkpoint` = `H_ckpt` of a versioned **Checkpoint object** (App A.2: covers frontier, retired heads, proof positions — D-77); `covers` = `H_frontier` of that same frontier (equality pinned). **The fence IS the covers frontier**: an operation's position is its signed `(lineage, gen, seq)` — covered iff ≤ that chain's head in `covers` (a pending operation occupies its signed coordinates). `proof_cutoffs` are tagged per issuer (`{ issuer, through }`). **Hardening (narrowed, D-77)**: a fence covering a pending operation's position AND every qualified witness feed hardens `deadline-unreceipted`/`lease-missing` to quarantine; `causal-missing` hardens when the fence covers the missing predecessor's coordinates and it is absent; `cert-superseded`/`policy-missing` never fence-harden (control-chain dependencies stay pending, owner-visible) | reject-permanent |
...
| **pending-dependency** (held until the referenced input arrives; fence-hardening per §7.1 `c.checkpoint` — deadline/lease/causal only, D-77) | causal-missing, cert-superseded (awaiting renewal chain), policy-missing, deadline-unreceipted, lease-missing (§9.1) |
...
across different flows. **Charge (D-75)**: the release charges its
canonical operation bytes plus the canonical bundle bytes against
the grant's `max_bytes`.
...
1 encoding+caps (incl. generated-size joint fits: 128 wraps + 128
manifest entries in one control op); 2 domains/key-ids;
3 signatures (low-S);
...
self-cutoff, lineage_reauth self-service, consumed request_ids
(`request-fork`), drill acceptance; zone-policy-advances-epoch +
two-replica arrival-order equivalence (epoch anchor); checkpoint
object + fence-coverage semantics; genesis grant completeness (the
15-verb set, inert raise/declassify, no deadline/lease); class table incl. mobile-attested vs mobile; renewal
...
equivalence); attestation `window_state` staleness; dense
...
exactness (indexes, disjoint union, multi-space scope) + result-id
domain + the 4096 edge cap; flow-deadline bound + whole-flow
matching; bundle-bytes accounting; export_id single-use; XferAbort
terminal (erased source);
...
each enumerated; tenant request-fork; multi-fault precedence); 13 storage
...
an epoch-1-committed item (D-73);
...
| D-68 | Mechanical batch: keyed-set logical identities (E7); chain arithmetic (seq from 1, +1; g = max + 1); genesis cross-field pins (provenance-compatible first cert class, grant epochs = 1, exact spaces/zone/lineage, wrap↔cert key equality, ordinary grant does NOT read audit space); `issued_admin_epoch` audit-only; zero-history `"none"` sentinel; enroll lineage absent-on-renewal; revocation exactness (complete grant set, rotation-after-last-wrap, any-order continuations); erase-manifest cap (superseded by D-77's regenerated caps); `c.cap_epoch_bump` body validation; `draw_order` {name, nbytes}; drift sweep (zone-qualified recovery cutoffs, enroll/reauth split, drill custody wording, device service writer, T4 receipt_cutoffs, `c.checkpoint` naming, §11.5 evidence ref) | Ratified, owner, 2026-07-12 |
| D-69 | Proof-policy anchor = the operation's **signed `capability_epoch`**: `policy(e)` = the ZonePolicy in force when epoch `e` opened; `c.zone_policy` acceptance advances the epoch (a policy change is an epoch event); epoch currency bounds staleness; compromise cutoffs are the sole revisit of admitted operations (→ quarantine-reproposal at the cutoff's fold position) | Ratified, owner, 2026-07-12 |
...
| D-72 | Requester attestations (`c.lineage_reauth`, `c.cutoff`) additionally bind `window_state` (the lineage's accepted `w.gen` count since last reauthorization/creation; admission requires equality) — banked attestations go stale as the window moves | Ratified, owner, 2026-07-12 |
...
| D-75 | Transfer authority: release deadline ≤ the matching flow's deadline; flow matching is existential and whole (one entry admits every axis); release charges op + bundle bytes against `max_bytes`; bundles are never persisted or framed (re-derived); plane-import header equality covers all three endpoint coordinates; egress proof = source-zone receipt on the release's own `item_addr`; bundle/bundlerec normative in App A.5 | Ratified, owner, 2026-07-12 |
...
checkpointobj = { v: 1, zone_id: ulid, covers: frontier,
  retired: [* head],                 ; set (E7), key (lineage, gen)
  proof_positions: [* { issuer: issuerid, through: uint }] }
                                     ; set (E7), keyed by issuer;
                                     ; H_ckpt(checkpointobj) is the
                                     ;   c.checkpoint `checkpoint` (D-77)
head = { lineage: bytes16, gen: uint, seq: uint, op: bytes32 }
...
clineagereauth = { lineage: bytes16, max_generations: uint,
  ...
  ;   max_generations, request_id, ctrl_frontier, window_state })) —
  ;   single-use (request_id consumed, D-59) and fresh (window_state
  ;   equality, D-72)
...
ccutoff = { cutoff: zonecutoff,
  ...
  ;   device_cert resolves to a certificate whose device_id owns the
  ;   target lineage; sig verifies under that certificate's key and
  ;   = Sign(device_sk, msg("cutoffreq", { plane_id, zone_id,
  ;   lineage, accepted_through, request_id, ctrl_frontier,
  ;   window_state })) — single-use (D-59), fresh (D-72)
cabandon = { zone_id: ulid, lineage: bytes16, at: head }
ccheckpoint = { zone_id: ulid,        ; zone-scoped (D-62)
  checkpoint: bytes32,                  ; = H_ckpt(checkpointobj), D-77
  covers: bytes32,                      ; = H_frontier(checkpointobj
                                        ;   .covers) — equality pinned;
                                        ;   the fence IS this frontier
  proof_cutoffs: [* { issuer: issuerid, through: uint }] }
                                        ; set (E7), keyed by issuer
