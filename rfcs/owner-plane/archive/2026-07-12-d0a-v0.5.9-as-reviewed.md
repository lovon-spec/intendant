...
*v0.5.9 draft (freeze candidate), 2026-07-12. Supersedes v0.1–v0.5.8
...
`~/agenda-rfc-archive/*d0a-v0.*-as-reviewed.md` (v0.1 through v0.5.8).
Folds the successive synthesized reviews (v0.4 through v0.5.8,
`~/owner-plane-d0a-spec-v0.5.8-synthesized-review.md` latest) with the
...
- **E8 (hard caps — deterministic parsing).**
  ...
  | held zones per device | ≤ 128 (D-109 — a grant/wrap creating the 129th rejects; `held_zones` defined once in §4.3, D-116/D-125 — control-derived, wildcard grants contribute nothing; keeps KEM renewal encodable) |
...
idempotent per `release_op`/`read_id` (D-123). Invalidation discovered after
...
**`held_zones(device)` (D-116/D-125, one definition, every
...
explicit wraps, each of which counts) and the zones where it holds
an effective (unsuperseded) wrap at any **accepted** epoch. The set
...
**effective-wrap subset** (the zones of the second kind — never
grant-only zones).
...
- **T3 (anti-replay and issuer lifecycle).** `issuer_seq` is
  ...
  **one counter per scope, shared by receipts and leases alike,
  ...
  hardening coverage. **One commitment registry per scope (D-124)**:
  ...
  NEW commitment MUST chain-ancestor-verify against that set
  (ancestor or descendant along one verified chain; `body-invariant`
  on the carrying operation otherwise), so two carriers can never
  commit opposite fork branches: carriers are control operations in
  one total order, and the first commitment above a fork point is
  the D-115 selection; **key renewal restarts the scope at 1** (a new
...
- **Cross-zone transfer (same daemon)** is two commits in two zone
  ...
  barrier), **except the release-rejection cleanup abort
  (D-119/D-126, `reason = "release-rejected"`)**: a reject-permanent
  **release** can never be effect-final, so its closing `XferAbort`
  is **journal cleanup**, outside the finality gate — it requires
  only that the release's rejection is immutable (a destination-side
  ...
  **unresolved** attempt and the terminal DEFERS until it resolves;
  once resolved reject-permanent/fence-hardened the record is
...
**Incorporation cap (D-107/D-114/D-122)**: a
...
(D-122)**: the cap exists for every **held** `w.gen` whose header
signature verifies under its named certificate — budget acceptance,
displacement, revival, and proof retro-disqualification never
...
wherever the bytes are, stable under every fold event, and
restriction-only, so over-inclusion fails closed). An unheld named
certificate leaves the cap pending (`ref-unresolved` lane) — it
clamps once resolvable. **Caps clamp within their generation, each
...
removes the cut branch's boundaries, and that is the owner's
explicit ceremony.
- **recover** (`c.recovery_succession.tenant_cutoffs`): branch-cut
  boundaries; immutable once accepted.

**Effective per-generation state (D-121 — the composition equation
every boundary event folds into).** Per `(zone, lineage, gen)`,
three components, each deterministic in the control order:

- **`ratified_through`** — the maximum `accepted_through` over the
  generation's accepted ratify entries (`"none"` when there are
  none); grows monotonically; clamped by `immutable_cap` at
  evaluation (an entry's excess above the cap is inert — see the
  incorporation-cap paragraph below).
- **`admit_bound`** — the revivable admission boundary, a fold over
  boundary events in control order: a ratify acceptance composes at
  the **maximum**; a requester snapshot's acceptance **overrides**
  ...
  both admitted and retired the same head). A later ratify above
  the override is genuine **growth** and revives; override then
  growth is the legal shrink-and-regrow cycle.
- **`immutable_cap`** — the **minimum** over the generation's
  accepted abandon seals and its incorporation caps (`"none"`/void
...
`p ≤ min(admit_bound, immutable_cap)`. Quarantine at
...
`m.import.claim`: import verb; `release_op + content_digest` must
match a verified release (D-123 — `export_id` rides along as
correlation). **v1 verification posture**: an integrated daemon
...
`class_floor`; replay keys on `(from_plane, release_op, source_op)`
(record-level, D-53/D-123) and is idempotent (`duplicate`). For plane
...
key; a gen/head mismatch rejects — D-120) + per-generation `"none"`
...
orders — D-121) + repeated-seal min-compose (`"none"` strongest —
...
history; the effective-wrap renewal set (a grant-only zone gets NO
renewal wrap and invalidates nothing — D-125) + the
effective-certificate wrap negative (a post-renewal wrap-add
targeting the predecessor KEM key rejects — D-125)); 8 recovery derivation; 9 time/lease (self-receipt
...
commitment at or below the fork selects neither — D-124); renewal feed_closure as a T3 boundary
...
cleanup vs `"reject-permanent"` terminal — D-126);
...
accepted, locally unretired epoch of every effective-wrap zone
holds Knew coverage (the active-only
predicate stranded queued intermediates and is void — D-116/D-125);
...
cenrollrenew = { cert: cert,                ; cert.renews REQUIRED
  ...
  history_cutoffs: [* zonecutoff],
      ...
      ;   effective-wrap set (§4.3, D-125 — the same set the
      ;   replacement wraps use; D-89)
  ? wraps: [* kekwrap] }
      ; REQUIRED iff the renewed cert rotates its KEM key: **one wrap
      ;   per zone of the device's EFFECTIVE-WRAP SET (§4.3, D-125 —
      ;   never grant-only zones: renewal mints no new access), at
      ...
      ;   predecessor KEM secret until EVERY accepted, locally
      ;   unretired epoch of every effective-wrap zone holds a Knew
      ;   wrap (queued epochs included; coverage via post-acceptance
...
ratifycutoff = { zone_id: ulid, lineage: bytes16, gen: uint,
  ...
  ;   (zone_id, lineage); "none" = that generation's zero-history
  ;   sentinel; entries compose at the maximum per key, clamped by
...
zoneheads = { zone_id: ulid, heads: [* head] }
  ...
  ;   per-generation entries (D-80/D-120);
...
bundle = { v: 1, export_id: bytes16, release_op: bytes32,
  recs: [+ bundlerec] }
  ; release_op = the authorizing release's op hash (D-123 — the
  ;   destination's replay-key input; export_id = correlation);
