...
*v0.5.11 draft (freeze candidate), 2026-07-12. Supersedes v0.1–v0.5.10
...
`~/agenda-rfc-archive/*d0a-v0.*-as-reviewed.md` (v0.1 through v0.5.10).
Folds the successive synthesized reviews (v0.4 through v0.5.10,
`~/owner-plane-d0a-spec-v0.5.10-synthesized-review.md` latest) with the
...
- **E8 (hard caps — deterministic parsing).**
  ...
  | recovery `tenant_cutoffs` | ≤ 256 (refinements — an omitted lineage folds the implicit revivable `"none"` override; no continuation needed, D-132/D-138) |
  | frontier heads | ≤ 4096 |
...
idempotent per the **effect key (D-134/D-142)**: a source
release's delivery keys by `("deliver", release_op)` and its
terminal record by `("terminal", release_op)` — namespaced, so a
...
- **T3 (anti-replay and issuer lifecycle).** `issuer_seq` is
  ...
  **Every non-genesis certificate REQUIRES a fresh signing key** —
  fresh **plane-wide and forever (D-131/D-141)**: an enrollment OR
  renewal whose `sig_pk`
  equals ANY certificate signing key ever enrolled on the plane
  rejects (renewal reuse would resurrect the predecessor's closed
  ...
  T2/T5); historical key knowledge is validation material and
  survives C3′ and compaction (the D-112 pattern) — the
...
- **Cross-zone transfer (same daemon)** is two commits in two zone
  ...
  **∧ effect finality** (D-94/D-101: acceptance authorizes; the
  ...
  identify a transfer. **Terminal basis and reopen (D-140)**: an
  `XferAbort` whose resolved-negative determination keyed on a
  boundary (a seal, void, or eligible cap) records that boundary's
  op hash as `basis`; when a later dissolution or C3′ removal
  invalidates the basis (D-128/D-131/D-138), the plane writer
  appends **`XferReopen { export_id, release_op }`** — the journal
  reopens and recovery re-derives (a terminal must be stable under
  every allowed future transition, never merely negative on the
  current branch — v0.5.10's terminals outlived removable bases);
  reject-permanent and source-erased terminals carry no `basis`
  and are stable by construction; **`XferDone` never reopens** —
...
| `c.space_retire` | admin | space retired — **an epoch event with MANDATORY closure (D-132/D-138)**: acceptance **advances the zone's capability epoch by 1** (§9.4) and binds the retirement to that epoch — a write naming the space and signed at or beyond it rejects (`scope-space`); because a writer can deliberately sign an older epoch (epoch staleness is not an admission predicate, and lenient zones keep old-epoch writing open by design), retirement additionally REQUIRES scalar close coverage of **every live lineage of the zone, regardless of strictness** (inline `closes` on the body ∪ staged `ccutoff.closes` — incomplete coverage rejects; old-epoch backdating dies on the immutable close boundary, not on the epoch anchor; v0.5.10's anchor-only rule was backdateable by design); reads/status unaffected | reject-permanent |
| `c.zone_policy` | admin | install full ZonePolicy (versioned; explicit — policy NEVER changes implicitly, incl. on witness arrival; owner ruling D-28); **acceptance advances the zone's capability epoch by 1 (D-69)** — operations signed at older epochs keep their old policy anchor (§9.4); `deadline_fallback = "fail-closed"` REQUIRES `require_cert_deadlines = true` (cross-field, D-76); **closure cutoffs (D-78/D-93/D-136)**: MAY carry `cutoffs` closing old-epoch writing per lineage — under `strict`, the **union** of this operation's entries and previously accepted CLOSE-purpose boundaries (staged `ccutoff.closes`; prior advances' entries) MUST cover every live lineage with an explicit **scalar** close (ratify state never counts — v0.5.10's some-generation-ratified coverage under-closed and ended no future old-epoch generation-opening; a >64-lineage zone stages `ccutoff.closes` first); each entry names this zone and a live lineage | reject-permanent |
...
| `c.cap_epoch_bump` | admin | zone capability epoch +1 — body `new_epoch` MUST equal the zone's current capability epoch + 1 (`body-invariant`; §9.4 consecutiveness, consumed at validation); opens a new budget window (D-79); closure `cutoffs` per the D-78/D-93/D-136 union-coverage rule (staged `ccutoff.closes` count — scalar close purpose only, never ratify state; each entry names this zone and a live lineage) | reject-permanent |
...
**except later selectors (D-137)**: a PENDING boundary that names a
coordinate RESERVES it — later boundaries naming that coordinate
(either variant) pend behind the reservation, released when the
reserving operation resolves reject-permanent or is removed by C3′
(v0.5.10 let a later committed selector win against an earlier
pending one: incremental folds selected the later boundary, fresh
folds the earlier — control order now survives pendency, the D-131
scope-reservation rule one domain wider). Bytes
...
different variant. **Scalable strict closure (D-136)**: coverage is
validated over the **union** of the advancing operation's own
entries (≤ 64) and previously accepted CLOSE-purpose boundaries —
staged `ccutoff.closes` and prior advances' entries; **ratify state
never counts** (v0.5.10's some-generation-ratified coverage
under-closed: it left other generations open and ended no FUTURE
generation-opening at the old epoch, which now dies on the scalar
close by lex order) — a 65-lineage zone stages `ccutoff.closes`
first, then
advances with the residue; operations arriving beyond a staged
close are quarantined-derived exactly as after the advance.
**Consumed-boundary promotion (D-111/D-136)**: coverage is always
an **explicit scalar close/supersede boundary per consumed
lineage** — inline on the consuming operation (`closure cutoffs` /
`history_cutoffs`) or staged (`ccutoff.closes`) — and promotion
materializes exactly those scalar values as immutable close
(advance) or supersede (renewal) state at the consuming control
position; the consumed prefix can never afterwards be widened
(the scalar is immutable and lex-total across generations, so it
also ends FUTURE generation-opening at the old epoch). v0.5.10's
per-generation freeze is **withdrawn (D-136)**: it both under-closed
(coverage-via-any-ratified-generation left sibling generations and
future generations open) and over-froze (it barred legitimate
below-close ratify revival, which never crosses the scalar
boundary); per-generation ratify state stays ordinary revivable
`admit_bound` state below the close. No duplicate wire value
is carried.
...
- **C3′ (recovery resolution, deterministic).**
  ...
  omission (D-132/D-138)**: `tenant_cutoffs` entries are explicit
  branch-cut boundaries — recover purpose, immutable — naming the
  history the owner PRESERVES (at or before each named head); a
  lineage OMITTED from the set folds an implicit **revivable
  `"none"` override**: every generation's `admit_bound` overrides
  to `Bounded("none")` at the recovery's fold position, quarantining
  ...
  ≤ 256 entries (E8) are refinements. C3′ acceptance also triggers
  the **total re-fold (D-138)**: all control-derived admission
...
|---|---|
| **reject-permanent** (never re-enters) | malformed, oversized, depth, non-canonical, sig-invalid, body-hash, body-invariant, key-malformed, proof-arm, no-grant, scope-*, class-*, provenance-ceiling, hosted-ceiling, no-flow, gen-first-op, request-fork (surfaced as fork evidence), import-collision (a later byte-distinct claim for a replay key already held in the zone's single import lineage — writer self-fault; the earlier-canonical claimant is immutable; D-134/D-139) |
...
(`duplicate`). **Import identity is FINAL (D-139)**: v1 pins **one
active `import`-verb grant per destination zone** (§7.1 `c.grant` —
a second active grant rejects at issuance, like the one-live-lineage
rule), so every import of a zone rides ONE lineage and the replay
key has a canonical total order — the first claimant in canonical
`(gen, seq)` order holds the key forever (a later operation can
never precede an earlier one in its own chain: no displacement, no
late winner, and referenced or effect-final imports are safe by
construction); a later byte-distinct claim against a held key is
`import-collision` (reject-permanent — writer self-fault,
deterministic on every replica). v0.5.10's lowest-`op_hash` rule is
...
requests target; identity must never move. Federated multi-writer
...
promotion as scalar materialization (the consuming advance's
close coverage — inline ∪ staged `closes` — materializes
immutably; ratify-as-coverage MUST fail; a post-advance `w.gen`
opening a NEW generation at the old epoch dies on the scalar
close; below-close ratify revival stays legal — D-136) +
exact-Head fork selection (H_A-first
and H_B-first replicas converge on the committed cutoff's named
branch; a second boundary naming the other variant rejects —
D-130) + the pending-selector reservation race (earlier-pending B1
vs later-committed B2 naming the other variant — B2 pends behind
the reservation; both orders + fresh fold converge; release on
B1's reject-permanent — D-137); supersede-boundary immutability (a later generic
...
(a renewal OR new-device enrollment reusing ANY historically
enrolled key rejects — A→B→A scope resurrection and the
D1-signs/D2-witnesses same-key bypass both die; historical keys
survive C3′ as validation material — D-131/D-141); renewal feed_closure as a T3 boundary
...
arrival orders — D-132); space retirement with mandatory closure
(a delayed pre-epoch write BELOW the close admits; a deliberate
old-epoch mint BEYOND the close dies on the boundary, not the
anchor; a coverage-missing retirement rejects; both arrival
orders — D-132/D-138));
...
acyclic, D-127); single-import-lineage identity (a second active import grant
rejects at issuance; the first canonical-order claimant is
immutable — a later byte-distinct claim is `import-collision`, and
a referenced, effect-final import can never be displaced — D-139);
effect-key granularity (one release, 128 imports
= 128 idempotency domains; deliver/terminal namespacing — D-134/
D-142); permanent-quarantine cleanup
(a release sealed beyond `immutable_cap` closes via the cleanup
abort; a permanently quarantined destination import becomes
eligible `missing` — D-134); terminal basis + reopen (an XferAbort
based on a seal whose branch a later C3′ cuts → `XferReopen`, the
journal re-derives; `XferDone` never reopens — D-140); the
`mimport` narrow-shape battery (one valid canonical import + one
negative per prohibited field + the mirror-matrix equalities
from_plane/digest/dest_zone/content_digest — D-142);
...
| D-120 | Ratify gets its own wire shape: `ratifycutoff = { zone_id, lineage, gen, accepted_through }`, logical key `(zone_id, lineage, gen)` (E7), `accepted_through.gen == gen` when a Head is present, per-generation `"none"` — v0.5.8's gen-less key collided with the per-generation algebra (two generations of one lineage were non-canonical in one operation, unencodable against D-80's multi-head promise); the immutable purposes stay on scalar `zonecutoff` deliberately (authority-ending, lex-comparator delimitation across generations); requester-present cutoffs bind the requester's own lineage on EVERY plane (one scalar `lineage_version`; trusted multi-lineage ceremonies go requesterless); requesterless cutoffs carry no snapshot boundary and skip the composition check — equation-only validation; the per-generation `"none"` re-ruled by D-129 (neutral requesterless, `body-invariant` in requester form; empty `cutoffs` legal with a requester); the `"none"` arm removed from `ratifycutoff` and the `closes` staging lane added by D-136 | Ratified, owner, 2026-07-12 |
...
cenrollrenew = { cert: cert,                ; cert.renews REQUIRED
  ...
  history_cutoffs: [* zonecutoff],
      ...
      ;   sentinel (the scalar zonecutoff KEEPS its "none" arm —
      ;   authority-ending purposes need it, D-136); zones beyond 64
      ;   are covered by pre-established staged closes
      ;   (ccutoff.closes, D-136 — ratify state never counts);
...
cspaceretire = { space_id: ulid, ? closes: [* zonecutoff] }
  ; retirement = an epoch event (S9.4) with MANDATORY scalar close
  ;   coverage of every live lineage regardless of strictness
  ;   (inline closes UNION staged ccutoff.closes — D-132/D-138);
  ;   writes signed at or beyond the retirement epoch reject
  ;   scope-space; old-epoch backdating dies on the close boundary
...
zonecutoff = { zone_id: ulid, lineage: bytes16,
  ...
  ;   a PENDING selector RESERVES its coordinate — later boundaries
  ;   naming it pend behind, released on reject-permanent or C3'
  ;   removal (D-137);
...
zoneheads = { zone_id: ulid, heads: [* head] }
  ...
  ;   body-invariant;
  ; closes: set (E7), key (zone_id, lineage), <= 64 — scalar CLOSE
  ;   boundaries (immutable once accepted, D-136): the staging lane
...
crecovsucc = { base: { seq: uint, op: bytes32 }, epoch: uint,
  ...
  tenant_cutoffs: [* zonecutoff],
      ...
      ;   (immutable, naming PRESERVED history); an OMITTED lineage
...
xferabort = { export_id: bytes16, release_op: bytes32,
  ...
  missing: [+ bytes32] }     ; set (E7), NON-EMPTY (D-98 — an empty
                             ...
                             ;   basis (D-140) = the boundary op a
                             ;   permanent-quarantine determination
                             ;   keyed on (absent for
                             ;   reject-permanent / source-erased —
                             ;   those are stable by construction)
xferreopen = { export_id: bytes16, release_op: bytes32 }
                             ; D-140: appended when a dissolution or
                             ;   C3' removal invalidates a terminal's
                             ;   basis — the journal reopens and
                             ;   recovery re-derives; XferDone never
...
mclaim = { kind: kind, statement: text, sensitivity: class,
  ...
  provenance: { ? session: text, ? project: text, ? model: text,
    ...
    ? import: { from_plane: bytes32, export_id: bytes16,
                ...
                ; v1 imports ride m.import.claim EXCLUSIVELY — an
                ;   m.propose/m.assert carrying provenance.import
                ;   rejects (body-invariant, D-142); the field is
                ;   the shared provenance vocabulary, populated only
                ;   by mimport's dedicated shape
...
mimport = { source_op: bytes32,   ; the released bundlerec this binds to
  ...
  ;   source_op) — D-123; export_id = correlation only; identity is
  ;   FINAL per the zone's single import lineage (D-139) — a later
  ;   byte-distinct same-key claim = import-collision
