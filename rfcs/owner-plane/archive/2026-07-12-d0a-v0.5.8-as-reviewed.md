...
*v0.5.8 draft (freeze candidate), 2026-07-12. Supersedes v0.1–v0.5.7
...
`~/agenda-rfc-archive/*d0a-v0.*-as-reviewed.md` (v0.1 through v0.5.7).
Folds the successive synthesized reviews (v0.4 through v0.5.7,
`~/owner-plane-d0a-spec-v0.5.7-synthesized-review.md` latest) with the
...
- **E8 (hard caps — deterministic parsing).**
  ...
  | recipients per (zone, accepted unretired epoch) | ≤ 256 (D-110/D-116 — enforced across EVERY wrap-bearing operation; a queued epoch is bounded too, so every possible Fence's `recipientset` stays constructible) |
  | held zones per device | ≤ 128 (D-109 — a grant/wrap creating the 129th rejects; `held_zones` defined once in §4.3, D-116; keeps KEM renewal encodable) |
...
**`held_zones(device)` (D-116, one definition, every consumer)**: the
union of the zones named by the device's active grants and the zones
where it holds an effective (unsuperseded) wrap at any accepted,
unretired epoch — the ≤ 128 cap (E8) counts this union; renewal
rewraps its effective-wrap subset.
...
- **T2 (witness qualification).** A receipt qualifies for an
  ...
  **The revisit inventory is derived, never counted (D-114 —
  ...
  retro-disqualification** — explicit compromise cutoffs (T4) and
  issuer-fork resolution (T3/D-115); (b) **canonical budget
  displacement and revival** (§4.3, D-86/D-94); and (c) **the
  boundary events of the cutoff algebra** (§7.1) — ratify growth
  revives, retirement boundaries (per-generation seals, snapshot
  cutoffs) quarantine. Nothing else revisits. Issuer-fork handling
  ...
  **both** suffixes at or after the fork freeze
  ...
  (`head_hash` cutoff, checkpoint proof position) then selects the
...
- **T3 (anti-replay and issuer lifecycle).** `issuer_seq` is
  ...
  **one counter per scope, shared by receipts and leases alike,
  ...
  ancestor proof, and the closed scope is exempt from checkpoint
  hardening coverage; **key renewal restarts the scope at 1** (a new
...
- **Cross-zone transfer (same daemon)** is two commits in two zone
  ...
  barrier), **except the reject-permanent cleanup abort (D-119)**: a
  reject-permanent release can never be effect-final, so its closing
  `XferAbort` is **journal cleanup**, outside the finality gate —
  it requires only that the release's rejection is immutable — recovery evaluates, in this order — after **rebuilding the
  ...
  **`export_id` critical section (D-113)**: the plane writer
  ...
  un-imported source_op **with no durable destination attempt**
  (D-119: a destination import that is durable but pending proof is
  an in-flight attempt — the terminal DEFERS until it resolves,
  admitted or reject-permanent/fence-hardened; `missing` may name
  only never-arrived or reject-permanent records — a delayed proof
  ...
  `content_digest` cannot be re-proven; committed imports stand; (3) else destination `reject-permanent`
  ...
  it revives with the release's re-acceptance (same `export_id`,
  same journal — the identity was never freed, D-119), and if the
  release resolves reject-permanent the journal closes with the
  cleanup `XferAbort` (`missing` = all un-imported records;
  journal cleanup outside the finality gate — liveness, not
  authority; D-119). **Terminal states
...
- **ratify** (`c.cutoff` only): grows the
  ratified-history boundary — **per generation (D-114)**: a
  `zonecutoff` binds exactly its named head's generation; multiple
...
**Incorporation cap (D-107/D-114)**: an accepted
`w.gen(last_known = H)` is itself an immutable boundary — it
permanently caps ratification of the incorporated generation at
`H`. **Caps clamp within their generation (D-114)**: a ratify
cutoff whose named head lies in an incorporated or sealed
generation and exceeds that generation's bound is `body-invariant`;
cutoffs in other generations are legal and unaffected (advancement
past a cap's generation is normal; revival past the bound inside it
is impossible by construction). **Boundary facts are monotone
(D-114)**: once accepted, an incorporation cap or seal persists
even if its creating operation is later displaced or
retro-quarantined (the displaced generation's suffix cascades per
the budget fold; effects that escaped on the strength of the
displaced operation fall into the stated residual).
...
| **quarantine-reproposal** (RFC §4.5 lane; a **derived** fold state, D-93/D-94 — ratify-boundary growth and displacement-charge release revive members deterministically. **Seal/void quarantine is permanent (D-114)**: operations beyond an abandonment seal or retired by a snapshot's uncarried-successor rule under a sealed/voided generation never revive — their content re-enters only as new operations; ratify-relative quarantine stays revivable) | capability-epoch, cutoff, lease-stale, budget, lineage-gen |
...
the stamp. **`export_id` is plane-wide single-use, consumed by acceptance
event (D-65/D-119)**: a release citing an export_id used by any
release **ever accepted** rejects (`body-invariant`) — later
displacement or dormancy never frees the identity (a displaced
release's revival must find its journal unshared); journal and
terminal records additionally carry the authorizing `release_op`. **Plane destinations are complete**: a plane
...
naming an uncarried generation — D-114); per-generation ratify
(same-generation revival past a cap rejects; later-generation
advancement legal — D-114); owner truncation (a seal below accepted
history + a generation void, both legal and product-surfaced —
D-114); adoption bound (65th entry rejects; deep-fork orphaning —
D-117); consumed-boundary
...
re-qualifies — D-115); renewal feed_closure as a T3 boundary
(ancestry gating + min-merge + hardening exemption — D-118); renewal feed closure (a delayed
...
accepted unretired epoch holds Knew coverage (the active-only
predicate stranded queued intermediates and is void — D-116);
...
256 cap (D-110/D-116); the durable-attempt deferral (a pending
import is never aborted missing — D-119); export_id
consumed-by-acceptance (displaced release revives into its own
journal — D-119);
...
kekwrap = { v: 1, plane_id: bytes32, zone_id: ulid, epoch: uint,
  ...
  ; GLOBAL key equality (D-116): in EVERY wrap-bearing operation
  ;   (genesis, zone-create, rotation, enroll, renewal, wrap-add),
  ;   recipient_kem_key MUST equal an enrolled certificate's KEM key
  ;   at acceptance — sole exception: the certificate enrolled by the
  ;   SAME operation (cgenesis/cenroll/cenrollrenew); no future-key
  ;   staging anywhere
...
cenrollrenew = { cert: cert,                ; cert.renews REQUIRED
  ...
  ? wraps: [* kekwrap] }
      ...
      ;   predecessor KEM secret until EVERY accepted, unretired
      ;   epoch of every held zone holds a Knew wrap (queued epochs
      ;   included; coverage via post-acceptance c.wrap_adds);
...
cwrapadd = { zone_id: ulid, epoch: uint, wrap: kekwrap }
  ...
  ;   key equality per the GLOBAL kekwrap rule (D-109/D-116); a wrap
  ;   creating a 257th recipient of any (zone, accepted unretired
  ;   epoch) rejects (D-110/D-116)
...
zoneheads = { zone_id: ulid, heads: [* head] }
  ...
  ;   zone, accepted_through <= that carried head (D-114); an unheld
...
pendingxfer = { export_id: bytes16, release_op: bytes32,
  dest_zone: ulid,
  content_digest: bytes32, record_count: uint }   ; release_op:
                                                  ;   D-119
