...
*v0.5.6 draft (freeze candidate), 2026-07-12. Supersedes v0.1–v0.5.5
...
`~/agenda-rfc-archive/*d0a-v0.*-as-reviewed.md` (v0.1 through v0.5.5).
Folds the successive synthesized reviews (v0.4 through v0.5.5,
`~/owner-plane-d0a-spec-v0.5.5-synthesized-review.md` latest) with the
...
`reauth`, `cutoffreq`, `stmtid`, `recips`, `survivors`. Derivation
domains:
...
effects to flow past an unknown gap **seals** it with
`c.abandon_writer` (immutable) instead of ratifying it. With the
...
effects until its cutoff — deterministic, and the deferral is
...
window; retired heads live in checkpoints, not the live
frontier. The live Frontier holds **accepted** heads only (D-80):
...
- **T2 (witness qualification).** A receipt qualifies for an
  ...
  **feed-fork exposure** (D-106): a later-committed feed boundary
  (cutoff `head_hash`, checkpoint proof position) revealing that a
  statement chain qualified operations on a losing branch
  retro-quarantines those operations at the commitment's fold
  position. Each is a fold of control state — deterministic on every
...
- **Cross-zone transfer (same daemon)** is two commits in two zone
  ...
  any source-side terminal is written, D-106): (1) **replay keys
  first**: every
...
| `c.grant` / `c.revoke_grant` | admin | issue / revoke capability; issuance to a device whose `revocation_id` is revoked **rejects** (`body-invariant`, D-92); **revoking an op-authoring grant carries a REQUIRED `cutoff` (D-78)** naming **that grant's zone and lineage** (equality, D-93) — a **revoke** boundary, immutable once accepted (no later ratify cutoff extends the revoked authority); operations at or before it stand, beyond it quarantine (`cutoff`); read-only revocation is edge-effective (no chain impact) | reject-permanent |
...
- **abandon** (`c.abandon_writer.at`): seals the lineage at the
  named head; **immutable once accepted** (D-101) — a later ratify
  cutoff may never exceed an abandonment boundary
  (`body-invariant`); abandonment is the effect-finality seal for
  unknown gaps.
...
- **C3′ (recovery resolution, deterministic).**
  ...
  { base: { seq: uint, op: bytes32 },
    ...
    adopted_rotations: [* { zone_id, rotation_op }] }
                                       # D-97: see below
  ```
  ...
  **Storage adoption (D-97/D-104)**: recovery whose base precedes an
  accepted `c.kek_rotate` either lists it in `adopted_rotations` —
  adopting a **specific Fence commitment** (`control_frontier` +
  `recipients_hash`, so two replicas that Fenced around different
  wrap-adds cannot both claim the adoption) together with the
  rotation's **entire predecessor chain** in that zone — or leaves
  it cut: a replica whose activation is unadopted **or mismatches
  the adopted commitment** transitions to outcome
  `storage-orphaned` (storage-quarantine: its post-Fence ciphertext
  is under an activation the surviving control state does not
  carry). All adoption references resolve **before** C3′ precedence
  (D-105); an unheld reference is `ref-unresolved` and confers no
  precedence while pending. The choice is the owner's, explicit and
  signed — recovery never silently strands activated storage.
...
- **(c)** admissible control operations: `c.enroll` (hosted-browser,
  ...
  **`c.lineage_reauth` and `c.cutoff` limited to the requesting
  ...
  non-empty erase manifest, no `c.abandon_writer` (D-15). Hosted planes therefore get
...
recovery storage adoption (adopted commitment matches → continues;
mismatched or unadopted activation → `storage-orphaned`; predecessor
chain adoption; unheld adoption reference = `ref-unresolved` with NO
precedence effect — D-104/D-105); abandon-seal vectors (ratify may
never exceed an abandonment; the seal confers effect finality —
D-101); supersede-boundary immutability (a later generic ratify
never widens a superseded certificate — D-102); comparator
...
after window consumption is LEGAL — D-100); feed-fork exposure (the
fourth revisit: a later-committed boundary retro-quarantines
losing-branch qualifications — D-106); dropped-witness
...
does NOT unlock it, `c.abandon_writer` does — D-101); the
64-open-gap cap negative (D-103); the grant-epoch
...
audited results / transfer terminals — D-101); frozen-stamp
...
latest-epoch renewal wraps (the unsatisfiable drain rule void —
D-104); `storage-orphaned` on mismatched/unadopted activation;
...
| Effect-finality gate (D-94/D-101) | displacement re-ordering an escaped external effect | bytes already escaped when a LATER compromise cutoff disqualifies the proofs they stood on (retro-quarantine recorded; the escape is unrecoverable — stated residual) |
...
| D-54 | Hosted self-`c.cutoff` (requester-attested, own lineage only) joins the ceiling; the grandfathered lane is a named, tested residual — `deadline_fallback` semantics since superseded by D-56 (issuance-time); attestation bytes per D-72 | Ratified, owner, 2026-07-12 |
...
cenrollrenew = { cert: cert,                ; cert.renews REQUIRED
  ...
  ? wraps: [* kekwrap] }
      ...
      ;   unchanged (old wraps stand) — D-89. Custody note (D-104):
      ;   the device retains the predecessor KEM secret until every
      ;   held zone's latest-epoch renewed wrap is accepted; grants
      ;   and lineage NEVER appear (D-84)
...
zonecutoff = { zone_id: ulid, lineage: bytes16,
  ...
  ; composition is scoped by BOUNDARY PURPOSE (D-93, superseding
  ;   D-92's global rule): ratify cutoffs compose at the maximum;
  ;   revoke/close/recover boundaries are immutable and never
  ;   composed — a generic cutoff cannot amend them;
  ; an unheld referenced Head = pending-dependency; a differing hash
...
zoneheads = { zone_id: ulid, heads: [* head] }
  ; heads: set (E7), key (gen, seq) — one lineage, ordered
  ...
  ;   live_heads is CARRIED (reconstruction invariant, D-100);
  ;   admission requires each named zone's live-head set to equal
  ;   the lineage's current one — assent invalidates the moment the
  ;   lineage writes past it; an unheld carried head pends
  ;   (ref-unresolved, D-102), never sig-invalid
cabandon = { zone_id: ulid, lineage: bytes16, at: head }
  ; boundary purpose ABANDON (D-101): immutable seal; later ratify
  ;   never exceeds it; the effect-finality closure for unknown gaps
...
crecovsucc = { base: { seq: uint, op: bytes32 }, epoch: uint,
  ...
  ; set (E7), keyed by zone_id (D-97/D-104): each entry adopts a
  ;   SPECIFIC Fence commitment — the named rotation AND its
  ;   activation state (control_frontier + recipients_hash, §5.5) —
  ;   plus the rotation's full predecessor chain in that zone (one
  ;   entry per zone = the highest adopted). A replica whose
  ;   activation matches continues; an activation that mismatches
  ;   the commitment, or of an unadopted rotation, is
  ;   `storage-orphaned` (→ storage-quarantine); an unheld
  ;   rotation_op is `ref-unresolved` (D-105: no precedence effect
  ;   while pending). The owner chooses, explicitly, per zone
...
recipientset = { pairs: [* { device: bytes16, kem_key: bytes32 }] }
  ; set (E7), keyed by device, <= 256 (D-106);
