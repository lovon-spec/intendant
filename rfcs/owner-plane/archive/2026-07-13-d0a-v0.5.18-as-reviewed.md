...
*v0.5.18 draft, 2026-07-13. Supersedes v0.1–v0.5.17
...
`~/agenda-rfc-archive/*d0a-v0.*-as-reviewed.md` (v0.1 through v0.5.17).
Folds the successive synthesized reviews (v0.4 through v0.5.17,
`~/owner-plane-d0a-spec-v0.5.17-synthesized-review.md` latest) with the
...
- **Cross-zone transfer (same daemon)** is two commits in two zone
  ...
  **`release_op` critical section (D-113/D-123)**: the plane writer
  ...
  attempt** (D-119/D-126/D-184/D-191: a destination import whose
  `ImportCommitted` marker precedes the source's tombstone in the
  source zone log — but which is
  not yet effect-final — is an
  **unresolved** attempt and the terminal DEFERS until it
  resolves; a MARKERLESS attempt at the tombstone —
  proof-pending included — resolves
  **`source-erased` at the erasure itself** (reject-permanent,
  D-184/D-191: the marker order is the boundary; v0.5.16 keyed on
  acceptance,
  ...
  **the cause table is ONE list (D-193 — v0.5.17 kept two
  ...
  proven-never-issued grants (D-194), and `source-erased`; basis
  ...
  **The journal object is frozen (D-192)**: the transfer journal is
  the zone log's txnrec projection **in frame order** — ONE append
  authority (the plane writer), the log's physical order the
  journal order, replay = fold frames in order (D0-B transports
...
admit(op, control_state, receipts):
  ...
  cert   resolve signer cert by device — **position-relative
         ...
         D-174/D-187) — and the dense-prefix reference proof
         (D-194): a cited certificate or grant ABSENT from the held
         dense control prefix covering the operation's signed
         anchor is PROVEN nonexistent (`no-cert`/`no-grant`,
         reject-permanent — C1 density makes absence portable); a
         gap below the anchor is `ref-unresolved` (unheld ≠
         nonexistent, on either delivery order)** —
...
the erasure boundary, carried by the `ImportCommitted` marker
(D-184/D-191)**: when source equality passes, the plane writer
appends **`ImportCommitted { source_op, release_op, leaf_hash }`**
to the **SOURCE zone log** — inside a `(source_zone, source_op)`
exclusion spanning the verification read through the marker append
(acquired BEFORE the `release_op` journal section where both are
held — a fixed nesting order; the exclusion is per source item, so
one source in many releases needs no discovery) — and the exact
erasure point is the item's **Tombstone append in the same log**:
an import survives the erasure iff its marker precedes the
tombstone **in that one log's frame order** — byte-visible,
crash-replayable, identical on every schedule (v0.5.17 asserted
the `release_op` section serialized commit against erasure, but no
erase transition takes that section, the destination ItemCommit
lives in a DIFFERENT zone log, and the lock key was wrong — one
source item appears in many releases; the verify→pause→erase→flush
schedule was legal and the bytes could not order it, D-191). A
markerless import resolves `source-erased` at the tombstone
(reject-permanent; eligible `missing`) — a proof-pending attempt
is markerless and dies identically; a crash after the marker but
before the destination commit re-commits the destination from the
held `mimport` bytes matched against the marker's `leaf_hash` (the
marker IS the verifier-authenticated durable record the withdrawn
v0.5.15 binding lacked a carrier for — its carrier is the log).
...
dense-prefix reference proof (a cited grant absent from the held
dense prefix covering the anchor = `no-grant` reject-permanent; a
control GAP below the anchor = `ref-unresolved` — both delivery
orders, D-194); the
...
D-179/D-185); the marker
schedules (B′ pending time proof →
source erased → the MARKERLESS attempt resolves `source-erased`
at the tombstone — the later proof changes nothing, and the
v0.5.14
after-erasure admission MUST fail; verify → marker → tombstone →
crash before the destination commit → recovery re-commits from
held `mimport` bytes against the marker's leaf_hash; the
verify→pause→erase→flush interleave is IMPOSSIBLE inside the
`(source_zone, source_op)` exclusion; one source in two releases —
two markers, independent survival; per-erase-state crashes and
cold replay of the ONE ordering log; both orders + fresh rebuild —
D-170/D-178/D-184/D-191);
...
txnrec = itemcommit / pendingxfer / xferdone / xferabort
       / xferreopen / importcommitted
importcommitted = { source_op: bytes32, release_op: bytes32,
  leaf_hash: bytes32 }
  ; D-191: appended to the SOURCE zone log by the plane writer when
  ;   source equality passes, inside the (source_zone, source_op)
  ;   exclusion (verification read → marker append; acquired before
  ;   the release_op section — fixed nesting); the import survives
  ;   erasure iff this marker precedes the item's Tombstone in the
  ;   SAME log's frame order; leaf_hash = the verified bundleleaf
  ;   hash (crash recovery re-commits the destination from held
  ;   mimport bytes matched against it)
