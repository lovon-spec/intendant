...
*v0.5.10 draft (freeze candidate), 2026-07-12. Supersedes v0.1–v0.5.9
...
`~/agenda-rfc-archive/*d0a-v0.*-as-reviewed.md` (v0.1 through v0.5.9).
Folds the successive synthesized reviews (v0.4 through v0.5.9,
`~/owner-plane-d0a-spec-v0.5.9-synthesized-review.md` latest) with the
...
- **E7 (sets and sort keys).** Every array documented as a *set* is
  ...
  zone/history/closure cutoffs (`zonecutoff`) →
...
- **E8 (hard caps — deterministic parsing).**
  ...
  | recovery `tenant_cutoffs` | ≤ 256 (refinements — an omitted cut-branch lineage quarantines entirely past base, revivably; no continuation needed, D-132) |
  | frontier heads | ≤ 4096 |
...
idempotent per the **effect key (D-134)**: a source release's
delivery and its terminal record key by `release_op`; each
...
- **T2 (witness qualification).** A receipt qualifies for an
  ...
  **The revisit inventory is derived, never counted (D-114 —
  ...
  displacement and revival** (§4.3, D-86/D-94); and (c) **the
  ...
  **widening event revives** — ratify growth (D-121) and
  ...
  window, D-132). **Removal is symmetric (D-131)**: a C3′ branch
  cut dissolving a fold input — a commitment, boundary, closure, or
  eligible cap on the cut branch — re-derives exactly what its
  presence derived (a fork whose committed selector dissolves has
  both suffixes refreeze pending a new selection).
...
- **T3 (anti-replay and issuer lifecycle).** `issuer_seq` is
  ...
  **Renewal therefore REQUIRES a fresh signing key** — fresh
  **plane-wide and forever (D-131)**: a `renews` whose `sig_pk`
  equals ANY certificate signing key ever enrolled on the plane
  rejects (not merely the immediate predecessor — A→B→A reuse would
  resurrect A's closed scope and its committed boundaries) — the
...
| `c.space_retire` | admin | space retired — **a position-relative boundary (D-132, the D-69 pattern)**: acceptance **advances the zone's capability epoch by 1** and binds the retirement to that epoch; a write naming the space admits iff its signed `capability_epoch` precedes the retirement epoch (a delayed pre-retirement write is thereby distinguishable from a post-retirement mint by its signed anchor — v0.5.9's positionless rejection was arrival-relative) and rejects (`scope-space`) at or beyond it; under `strict`, closure `cutoffs` per the D-78/D-93 union-coverage rule, exactly as every epoch-advancing operation; reads/status unaffected | reject-permanent |
...
- **ratify** (`c.cutoff` only): grows the
  ...
  confers effect finality (D-101). The remaining purposes stay on
  the gen-less scalar `zonecutoff` **deliberately (D-120)**: they
  are authority-ending, fail-closed boundaries delimiting by the
  total D-102 comparator — lex `(gen, seq)`, one scalar head ending
  everything beyond it across generations, which is exactly their
  job; only ratify GRANTS, and only granting needed per-generation
  precision.
...
terminal-Head validity of `last_known`. Every input is a portable
fold of held bytes and control history — never budget state. An
unheld eligibility input (certificate, grant, chain prefix,
terminal Head) leaves the cap **pending** (`ref-unresolved` lane) —
it clamps once resolvable. **Dissolution (D-128)**: an explicit
compromise cutoff retro-disqualifying the `w.gen`'s certificate at
its position, or a C3′ branch cut removing its branch (D-131),
dissolves the cap and re-derives what it clamped — the owner's
remedy against an adversary-minted cap; an effect that escaped
...
`w.gen`'s budget outcome; cap-bearing bytes are **GC-exempt
boundary evidence** (bounded by the 64-open-gap E8 cap — retention
is checkpoint/compaction-proof). **Caps clamp within their
generation, each
...
cap (`ratified_through` and `admit_bound` evaluate `min` the cap,
D-121; v0.5.8's `body-invariant` here was arrival-relative: the
...
**Effective per-generation state (D-121/D-129 — the composition
equation
every boundary event folds into).** Per `(zone, lineage, gen)`,
three components, each deterministic in the control order:

Boundary values live in the ordered domain
`Absent < "none" < Head(H) < Top` — **`Absent`/`Top` are fold
identities, never wire values (D-129)**: `Absent` = no event yet
(the max-fold identity), `Top` = unbounded (the min-fold identity);
wire `"none"` is an **explicit assertion**, never a default (E4).

- **`ratified_through`** — initial `Absent`; max-composes the
  generation's accepted ratify entries; grows monotonically;
  clamped by `immutable_cap` at
  evaluation (an entry's excess above the cap is inert — see the
  incorporation-cap paragraph above). **Its consumers (D-129)**:
  the coverage predicates — the strict-closure union (§7.1) and
  renewal history coverage read whether ratify state exists for a
  `(zone, lineage)` (`ratified_through ≠ Absent` on some
  generation) — and D-111 promotion, which freezes its
  per-generation values; admission reads the pair below, never
  this component directly.
- **`admit_bound`** — initial `Top`; the revivable admission
  boundary, a fold over
  boundary events in control order: a ratify acceptance
  max-composes its entry value; a requester snapshot's acceptance
  **overrides**
  to its carried head for every generation it carries — this is the
  retirement event: positions beyond the carried head quarantine at
  that fold position even when a larger ratify preceded it (v0.5.8
  left the ratify-max/snapshot interaction undefined — a boundary
  both admitted and retired the same head); **entry/snapshot
  coherence is equality (D-129)**: a requester entry MUST equal its
  generation's carried head (`body-invariant` below or above it,
  and wire `"none"` is `body-invariant` in requester form — a
  carried generation has a Head by construction, and zero-history
  assertion is seal/void authority, never ratify's) — the entries
  ...
  growth is the legal shrink-and-regrow cycle. In requesterless
  form an entry max-composes directly, and `"none"` is the
  **neutral element** (ratifies the empty prefix; never retires —
  D-129).
- **`immutable_cap`** — initial `Top`; the **minimum** over the
  generation's
  accepted abandon seals, its eligible incorporation caps (D-128),
  and its D-111-promoted values (D-130) (`"none"`/void
  strongest; repeated seals min-compose here — the same rule
  receipt cutoffs use); never grows, never composed with the other
  two — it clamps them.

Effective admission for a position `p` is
`p ≤ min(admit_bound, immutable_cap)` (`Top` admits everything the
chain and budget rules otherwise admit; `Absent` never appears in
this pair). Quarantine at
`p > immutable_cap`, or anywhere in a voided generation, is
**permanent**; quarantine at `admit_bound < p ≤ immutable_cap` is
...
a pending reference (only the referencing operation waits); bytes
...
different variant. **Scalable strict closure**: coverage is
...
**Consumed-boundary promotion (D-111/D-130)**: when an epoch
advance or a
renewal consumes ratify state for its coverage, the consumed
lineage's **per-generation map is materialized at the consuming
control position** as immutable close (advance) or supersede
(renewal) state: each generation with an accepted ratify boundary
at that position min-folds its value into that generation's
`immutable_cap` — the exact no-widening reduction (a scalar
lex-maximum was rejected: it leaves lex-below generations widenable
and re-widens exactly what D-111 froze); generations without ratify
state freeze nothing and remain governed by certificate
supersession and the feed closure (named residual). Coverage for a
`(zone, lineage)` is satisfied by an explicit scalar entry
(`history_cutoffs` / closure `cutoffs`) or by any accepted ratify
state (`ratified_through ≠ Absent`, D-129); no duplicate wire value
is carried.
...
- **C3′ (recovery resolution, deterministic).**
  ...
  omission (D-132)**: `tenant_cutoffs` entries are explicit
  branch-cut boundaries — recover purpose, immutable; a cut-branch
  lineage OMITTED from the set quarantines **entirely past `base`**
  (fail-closed blanket), and the blanket — unlike the named
  entries — is **revivable** by post-recovery ratify growth
  (D-121), so a wide plane needs no continuation operation: the
  ≤ 256 entries (E8) are refinements, the blanket covers the rest
  of the ≤ 4096 heads. A recovery also **dissolves the cut branch's
  fold inputs (D-131)**: commitments, boundaries, closures, and
  eligible caps riding cut operations are removed and everything
  their presence derived re-derives.
  **Storage adoption (D-97/D-104/D-112)**: recovery whose base
  ...
  (nothing activated — re-issue on the surviving branch). All adoption references
...
|---|---|
| **reject-permanent** (never re-enters) | malformed, oversized, depth, non-canonical, sig-invalid, body-hash, body-invariant, key-malformed, proof-arm, no-grant, scope-*, class-*, provenance-ceiling, hosted-ceiling, no-flow, gen-first-op, request-fork (surfaced as fork evidence), import-collision (the lower-`op_hash` claimant holds the replay key — content is fully derived, so the loss is semantically invisible; D-134) |
...
`release_op` was an infeasible fixed point: `release_op` =
`H_op`(release containing `content_digest` = `H_bundle`(bundle
containing `release_op`)) — no producer could mint the release).
The signed release triple **travels alongside** the bundle; the
destination verifies the release chain, checks
`H_bundle(received bundle) == release.content_digest`, and
...
(`duplicate`), and **byte-distinct claimants for one replay key
resolve by lowest `op_hash` (`import-collision`, D-134)**: the
loser is reject-permanent (its charge releases; an accepted loser
displaced by a later-arriving lower hash re-derives exactly like
budget displacement) — cross-lineage claimants have no canonical
order, so the hash is the key's total order, and derived content
makes the choice semantically invisible; identical on both arrival
orders and on a fresh fold. **Mirror equality (D-134)**: one
transfer's `export_id` is byte-equal across release, bundle,
journal records, `provenance.import`, and terminals; its
`release_op` — everywhere the derived value, never signed input
(D-127) — is byte-equal across journal records,
...
key; a gen/head mismatch rejects — D-120) + requesterless-neutral
`"none"` (the max-compose identity, never retirement) vs the
requester-form equality negatives (`"none"`, below-head, above-head
— all `body-invariant`, D-129) + the pure-snapshot ceremony (empty
`cutoffs` with an empty carried `heads` entry — the representable
zero-history form; the requesterless-empty negative — D-129)
...
orders — D-121) + `Absent`/`Top` identity probes (no boundary event
is distinct from explicit `"none"` — D-129) + repeated-seal
...
`w.gen` does; a compromise cutoff dissolves an adversary cap and
the clamped suffix re-derives — D-128);
...
promotion as the per-generation freeze (a three-generation map
`{g1: H10, g2: absent, g3: H5}` min-folds each ratified value into
its generation's `immutable_cap`; the scalar lex-max counter-case —
g1 growth past H10 lex-below (g3, H5) — MUST fail — D-111/D-130) +
...
naming the other variant rejects — D-130); supersede-boundary immutability (a later generic
...
stands and Kold drains as its old-wrap epochs retire — D-133) + the
effective-certificate wrap negative (a post-renewal wrap-add
targeting the predecessor KEM key rejects — D-125)); 8 recovery
derivation (omitted-lineage blanket: a cut-branch lineage absent
from `tenant_cutoffs` quarantines entirely past base and revives
under post-recovery ratify growth; a >256-lineage plane needs no
continuation — D-132; selector dissolution: a branch cut removing a
committed fork selector refreezes both suffixes and dissolved
caps/closures re-derive — D-131/D-128); 9 time/lease (self-receipt
...
through pendency — D-124/D-131) + plane-wide signing-key freshness
(a renewal reusing ANY historically enrolled key rejects — the
A→B→A scope resurrection dies — D-131); renewal feed_closure as a T3 boundary
...
arrival orders — D-132); space retirement as an epoch event (a
delayed write signed at a pre-retirement epoch admits, a
post-epoch mint rejects `scope-space`, both arrival orders —
D-132));
...
acyclic, D-127); byte-distinct same-key import claimants (lowest
`op_hash` wins on both arrival orders and a fresh fold; derived
content — D-134); effect-key granularity (one release, 128 imports
= 128 idempotency domains — D-134); permanent-quarantine cleanup
(a release sealed beyond `immutable_cap` closes via the cleanup
abort; a permanently quarantined destination import becomes
eligible `missing` — D-134);
...
| D-53 | Record-level transfer identity `(from_plane, export_id, source_op)`; PendingXfer carries `content_digest` + `record_count`; one terminal record per release; bundles re-derived deterministically on recovery — terminality extended by D-65 (XferAbort); replay key re-keyed by `release_op` (D-123) | Ratified, owner, 2026-07-12 |
...
cenrollrenew = { cert: cert,                ; cert.renews REQUIRED
  ...
  history_cutoffs: [* zonecutoff],
      ...
      ;   requires the UNION to cover every zone of the device's
      ;   current-membership set (§4.3, D-125/D-133 — the same set
      ;   the replacement wraps use; D-89)
  ? wraps: [* kekwrap] }
      ...
      ;   a Knew wrap or retires locally (absence of a wrap at an
      ;   epoch never obliges one — an excluded device's Kold drains
      ;   as its old-wrap epochs retire, D-133; queued epochs it
      ;   holds wraps at ARE included; coverage via post-acceptance
      ;   c.wrap_adds; custody is LOCAL state — admission never
      ;   reads it, D-125);
...
zonecutoff = { zone_id: ulid, lineage: bytes16,
  ...
  ; the SCALAR shape of the immutable, authority-ending purposes —
  ;   supersede / revoke / close / recover (D-120: deliberate; they
  ;   delimit by the total D-102 lex comparator, one head ending
  ;   everything beyond it across generations; ratify rides its own
  ;   per-generation ratifycutoff; abandon rides cabandon.seals);
...
ratifycutoff = { zone_id: ulid, lineage: bytes16, gen: uint,
  ...
  ;   (zone_id, lineage, gen) (E7); equality: a present Head has
  ;   accepted_through.gen == gen and belongs to its named
  ;   (zone_id, lineage); "none" = the NEUTRAL element (ratifies
  ;   the empty prefix, never retires — legal requesterless only;
  ;   body-invariant in requester form, where every entry must
  ;   EQUAL its generation's carried head — D-129); entries compose
  ;   at the maximum per key, clamped by
...
zoneheads = { zone_id: ulid, heads: [* head] }
  ...
  ;   requester is present (pure snapshot retirement — the
  ;   representable zero-history form; requesterless empty =
  ;   body-invariant, D-129);
...
crecovsucc = { base: { seq: uint, op: bytes32 }, epoch: uint,
  ...
  tenant_cutoffs: [* zonecutoff],
      ; set (E7), <= 256 (E8) — explicit recover boundaries
      ;   (immutable); an OMITTED cut-branch lineage quarantines
      ;   entirely past base (fail-closed blanket, REVIVABLE by
      ;   post-recovery ratify growth — D-132; entries are
      ;   refinements, no continuation op needed)
...
mclaim = { kind: kind, statement: text, sensitivity: class,
  ...
  provenance: { ? session: text, ? project: text, ? model: text,
    ...
    ? import: { from_plane: bytes32, export_id: bytes16,
                release_op: bytes32, digest: bytes32 } },
...
mimport = { source_op: bytes32,   ; the released bundlerec this binds to
  ...
  ; v1 content fully derived (D-134): sensitivity == class_floor,
  ;   temporal fields ABSENT, provenance = exactly the import
  ;   triple, labels/evidence ABSENT (annotate post-import);
  ; replay keys on (provenance.import.from_plane, release_op,
  ;   source_op) — D-123; export_id = correlation only;
  ;   byte-distinct same-key claimants: lowest op_hash wins, the
  ;   loser = import-collision (D-134)
