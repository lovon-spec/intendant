...
*v0.5.12 draft, 2026-07-13. Supersedes v0.1–v0.5.11
...
`~/agenda-rfc-archive/*d0a-v0.*-as-reviewed.md` (v0.1 through v0.5.11).
Folds the successive synthesized reviews (v0.4 through v0.5.11,
`~/owner-plane-d0a-spec-v0.5.11-synthesized-review.md` latest) with the
...
- **E8 (hard caps — deterministic parsing).**
  ...
  | recovery `tenant_cutoffs` | ≤ 256 (refinements — an omitted `(zone, lineage)` pair folds the implicit revivable `"none"` override; no continuation needed, D-132/D-138/D-151) |
  ...
  | `frontierclose.heads` | ≤ 64 (D-143 — the open-gap cap bounds live heads) |
  | `mimport.proof` | ≤ 8 (D-147 — 1 + 128 leaves → depth ≤ 8) |
  | frontier heads | ≤ 4096 |
...
`survivors`, `bhdr`, `brec`, `bnode` (the bundle Merkle domains,
D-147 — the flat `bundle` domain is retired with `H_bundle`). Derivation
domains:
...
- **T3 (anti-replay and issuer lifecycle).** `issuer_seq` is
  ...
  **Every non-genesis certificate REQUIRES a fresh signing key** —
  ...
  T2/T5). **The freshness domain is portable (D-150)**: the
  surviving chain's enrollments ∪ the `retired_keys` committed by
  accepted recoveries (v0.5.11's fork-inclusive "ever enrolled"
  read cut-branch certificates some replicas never hold — the
  freshness check itself diverged; a cut-branch key the owner does
  NOT carry in `retired_keys` is re-usable — a stated, vectored
...
- **Cross-zone transfer (same daemon)** is two commits in two zone
  ...
  **∧ effect finality** (D-94/D-101: acceptance authorizes; the
  ...
  identify a transfer. **Terminal basis and reopen (D-140/D-148)**: the journal is a
  **transition fold** — one EFFECTIVE terminal per interval,
  intervals chained by `XferReopen`. Each `missing` entry carries
  its own optional `basis` — the boundary op (seal, void, eligible
  cap) its resolved-negative determination keyed on; **basis
  presence IS the discriminator**: an intrinsic rejection
  (reject-permanent bytes, source-erased) carries none and is
  stable by construction, a boundary-derived one carries its cause
  (v0.5.11's single scalar `basis` could not arbitrate two records
  negative under independent boundaries). When a dissolution or C3′
  removal invalidates ANY recorded basis (D-128/D-131/D-138), the
  plane writer appends **`XferReopen { export_id, release_op,
  incarnation }`** — `incarnation` = the index of the terminal
  interval it closes — and recovery re-derives **per record**:
...
| `c.cap_epoch_bump` | admin | zone capability epoch +1 — body `new_epoch` MUST equal the zone's current capability epoch + 1 (`body-invariant`; §9.4 consecutiveness, consumed at validation); opens a new budget window (D-79); closure `cutoffs` (`frontierclose`s) per the D-78/D-93/D-136/D-143 union-coverage rule (staged `ccutoff.closes` count — frontier closes only, never ratify state; each entry names this zone and a live lineage, D-151) | reject-permanent |
...
**except later selectors (D-137/D-145)**: a PENDING boundary that
names a Head RESERVES the full **`(zone, lineage, gen)` selection
scope** — later boundaries naming ANY coordinate in that generation
...
v0.5.11 reserved only the exact coordinate, so a later selector
naming a DESCENDANT on the opposite branch still committed before
the earlier-in-control-order reference resolved — with the
ancestry unresolved, no narrower safe cone is knowable, so the
whole generation waits — the D-131
...
different variant. **Scalable strict closure (D-136/D-143)**: coverage is
validated over the **union** of the advancing operation's own
entries (≤ 64) and previously accepted staged frontiers
(`ccutoff.closes`; prior advances' entries); **ratify state
...
by composition.
**Consumed-boundary promotion (D-111/D-136/D-143)**: coverage is
...
is **inert until consumed** — coverage material only; the consuming
operation (epoch advance, renewal, retirement) MATERIALIZES it as
an authority closure with the **consumer's selector** at the
consuming control position (inline entries materialize
identically), and the consumed boundary can never afterwards be
...
content_digest = the MERKLE ROOT (D-147):
  leaves = H_bhdr({v, export_id, record_count})   # the header leaf
        then H_brec(canonical bundlerec bytes),   # one per rec,
                                                  #   canonical order
  parents = H_bnode(left || right); an odd node promotes
```

**Per-record durable proof (D-147)**: v0.5.11's flat
...
Each `mimport` now carries its leaf's `rec_index` and Merkle
`proof` path (≤ 8 siblings under the 128-rec cap): folding
`H_brec(record)` up the path MUST reach `release.content_digest` —
per-record, durable, erasure-proof validation material.
...
destination verifies the release chain, re-derives the Merkle root
from the received bundle (or verifies per-record paths, D-147), and
...
is **provisional until the claimant is effect-final, and frozen
there (D-146)** — D-94's finality closure IS the no-late-claimant
proof: once every lower generation of the import lineage is
immutably closed, no canonically earlier claimant can arrive
(v0.5.11's one-active-grant lemma alone was false twice over —
grant turnover put claimants on incomparable chains, and one
lineage's open unknown-gap generations admitted late canonically
EARLIER claimants). Pre-finality, a canonically earlier claimant
DISPLACES a provisional one (standard displacement — the charge
releases, the suffix re-derives), and judgments, pins, or erase
requests targeting a provisional import claim are
**pending-dependency** until it freezes; a claim that can never win
— later in canonical order against a held claim, or against an
effect-final claimant — is `import-collision` (reject-permanent).
Grant handoff serializes on the revoke boundary: the mandatory
revocation frontier ends the old lineage's import authority — its
provisional claims beyond the boundary die with it, its effect-final
claims stand immutable, and a new-lineage claim against an
effect-final-held key is `import-collision`. v0.5.10's
...
requests target; a FROZEN identity never moves. Federated
multi-writer
...
bricked them); a post-advance `w.gen`
...
vs later-committed B2 naming the other variant OR a descendant
coordinate in the same generation — B2 pends behind
the scope reservation; both orders + fresh fold converge; release
on B1's reject-permanent — D-137/D-145) + the cap-anchor race (W =
`w.gen(last_known = H5)` vs delayed successor H6: W-first,
H6-first, and fresh folds all converge on the cap winning —
chain-membership, never terminality — D-144); supersede-boundary immutability (a later generic
...
>256-lineage plane needs no continuation — D-132/D-138/D-151; the
total
...
D-131/D-138/D-128; KEM-renewal adoption: C3′ with base before a
completed renewal either adopts it — `adopted_renewals` — or the
K0-destroyed device is `storage-orphaned`, never silently
re-keyed — D-150); 9 time/lease (self-receipt
...
acyclic, D-127); provisional-until-final import identity (a second active import
grant rejects at issuance — on `c.grant` AND `c.enroll.grants[]`;
a late canonically EARLIER claimant in a still-live lower
generation DISPLACES a provisional import, and a judgment/pin/
erase targeting it pends until effect finality freezes the
claimant; grant revoke-and-reissue to another lineage — and its
C3′ equivalent — serializes on the revocation frontier; a claim
against an effect-final-held key is `import-collision` — D-139/
D-146);
...
its record; Abort→Reopen→Done through a real consume-once dedupe
store — the incarnation key admits the successor terminal;
`XferDone` never reopens — D-140/D-148); the Merkle-proof battery
...
crevokedev = { mode: "exclude" / "compromise",
  ...
  cutoffs: [+ frontierclose],                          ; set; one per zone
  ? receipt_cutoffs: [+ { key_id: bytes32, through: uint,
                          head_hash: bytes32 }],
...
frontierclose = { zone_id: ulid, lineage: bytes16,
  ...
  ;   one per generation, Frontier-consistent), <= 64 (the open-gap
  ;   cap bounds live heads); TOTAL-OVERRIDE semantics for the
  ...
  ;   a PENDING selector RESERVES the full (zone, lineage, gen)
  ;   scope — later boundaries naming ANY coordinate in that
  ;   generation pend behind, released on resolution,
  ;   reject-permanent, or C3' removal (D-137/D-145);
...
zoneheads = { zone_id: ulid, heads: [* head] }
  ...
  ;   frontier closures (D-136/D-143): INERT until a consuming
  ;   advance/renewal/retirement materializes them with its
  ;   selector; the staging lane
...
crecovsucc = { base: { seq: uint, op: bytes32 }, epoch: uint,
  ...
  tenant_cutoffs: [* frontierclose],
      ...
      ;   the complement domain is (zone, lineage) PAIRS (D-151): a
  ...
  ? adopted_renewals: [* { device_id: bytes16,
      ...
      ;   and storage state (the device's recipient identity) — or
  ...
  ? retired_keys: [* bytes32],
      ...
      ;   signing/KEM freshness domain; an uncarried cut-branch key
...
xferabort = { export_id: bytes16, release_op: bytes32,
  ...
  missing: [+ { rec: bytes32, ? basis: bytes32 }] }
                             ...
                             ;   the boundary op THIS record's
                             ;   permanent-quarantine determination
                             ;   keyed on (absent = intrinsic —
                             ;   reject-permanent bytes /
                             ;   source-erased — stable by
                             ;   construction; basis presence IS
                             ;   the discriminator)
xferreopen = { export_id: bytes16, release_op: bytes32,
  incarnation: uint }
                             ; D-140/D-148: appended when a
                             ;   dissolution or C3' removal
                             ;   invalidates ANY recorded basis —
                             ;   incarnation = the terminal
                             ;   interval it closes (0-based); the
...
mimport = { source_op: bytes32,   ; the released bundlerec this binds to
  ...
  rec_index: uint,                  ; the record's leaf position
  proof: [* bytes32],               ; Merkle siblings, <= 8 (E8) —
                                    ;   H_brec(record) folded up
                                    ;   proof reaches
                                    ;   release.content_digest
                                    ;   (D-147: per-record durable
                                    ;   validation — survives source
                                    ;   erasure and rebuild)
  provenance: {
