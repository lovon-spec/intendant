...
*v0.5.15 draft, 2026-07-13. Supersedes v0.1–v0.5.14
...
`~/agenda-rfc-archive/*d0a-v0.*-as-reviewed.md` (v0.1 through v0.5.14).
Folds the successive synthesized reviews (v0.4 through v0.5.14,
`~/owner-plane-d0a-spec-v0.5.14-synthesized-review.md` latest) with the
...
- Renewal: `c.enroll` with the same `device_id` + `renews`, carrying
  ...
  **legitimately omitted** from coverage (its last grant already
  ...
  temporal compatibility (D-174)**: a superseded certificate never
  admits an operation citing a grant issued AFTER its supersession
  (`cert-superseded` — the old key authors nothing new, §7.1). Any replay, incremental or
...
- **T3 (anti-replay and issuer lifecycle).** `issuer_seq` is
  ...
  **Every non-genesis certificate REQUIRES a fresh signing key** —
  ...
  rejects (intra-certificate role reuse); the domain = the
...
- **Cross-zone transfer (same daemon)** is two commits in two zone
  ...
  **transition fold** — one EFFECTIVE terminal per interval,
  ...
  (D-157/D-163/D-171): among the branch-relative facts sufficient
  at or before the terminal's committed evaluation coordinate
  `at`, the MINIMAL op hash** — a pure state
  function, reconstructible on any rebuild (v0.5.13's "first in
  fold order" had no total
  cross-feed order; v0.5.14 defined the position but carried no
  coordinate, so a rebuild could not know which facts existed
  then): a control boundary (seal, void,
  eligible cap, retirement, revocation) OR a tenant fact — the
  closed cause
  vocabulary is exactly D-149's branch-relative class, mapped
  outcome by outcome and TOTAL (D-163/D-171): `cutoff` and
  `scope-*` → the
  ...
  `body-invariant` instances → the contradicting control op;
  `import-collision` → **the typed conjunction
  `(winner, freeze_basis)` (D-169)** — the loser is a collision
  only BECAUSE the winner is frozen, so the terminal invalidates
  when EITHER member dies (v0.5.14 recorded the winner alone: the
  freeze basis's removal never invalidated the terminal);
  intrinsic byte failures
...
(D-174)**: an operation citing certificate C and grant G admits
only if G's issuance position falls within C's effective span (G
was issued while C was the device's effective certificate) OR C
holds a supersede frontier covering the operation's position —
otherwise `cert-superseded` (absence-as-neutral alone let a
revoked-G0 / renewed-C0 / freshly issued G1 resurrect the OLD
signer in a legitimately omitted zone: grants bind the device,
not a certificate generation, so no closure matched anywhere —
"the old key authors nothing new" now holds by construction; both
inputs are control facts, fully portable). §4.2 and the §10.2 cert
...
consumer** — a strict-zone advance or ANY space retirement (whose
coverage is mandatory regardless of strictness, D-138) — whose
coverage
...
transition); a stage whose lineage has left the coverage domain by
...
admit(op, control_state, receipts):
  ...
  cert   resolve signer cert by device — **position-relative
         ...
         (a superseded cert citing a post-supersession grant =
         `cert-superseded`, D-174)** —
...
**One validator (D-162/D-170)**: admission is **per-record,
always** —
the leaf + path against the signed root, plus byte equality of
`rec` against the **source-derived binding**: the durable
destination attempt RECORDS the source-derived leaf hash at its
registration (a durable attempt cannot exist without it — the
release chain and source record were verified to create it), and
body validation compares against the STORED binding whenever the
source is no longer derivable (D-170 — v0.5.14's while-derivable
conditional was availability-dependent: a malicious B′ pending
time proof, its source erased, then proven, skipped equality and
imported on its valid path; the stored binding makes the check
erasure-stable — the signed-leaf-authoritative alternative was
REJECTED as a security downgrade: an export-authorized signer
...
per-record rule the answer never changes — A imports on
its own proof and source binding both times, B′ never imports:
its source-derived binding mismatches on every schedule).
...
reject-permanent or frozen-out claimants release the reservation,
...
wraps-only completion MUST fail; 0-author/65-wrap under the
DECRYPTABLE-WRAP domain: prior exclusion rotations shrink it,
`wrap_add` re-admission re-enters it pre-completion — D-165/
D-173);
...
`kem_pk` are one point rejects — D-175) + the
...
against a frozen owner is `import-collision`, its terminal cause
the (winner, freeze_basis) conjunction — the freeze basis dying
reverts the loser to displaced-revivable via reopen — D-139/
D-146/D-155/D-161/D-169);
...
log-corrupt — D-163); the rebuild-at-coordinate trace (a later
lower-hash sufficient fact exists — the terminal's `at` pins the
evaluation and the fresh rebuild re-derives the SAME basis —
D-171); the stored-binding schedule (B′ pending time proof →
source erased → proof arrives → body validation compares the
STORED source-derived leaf and rejects — the v0.5.14
after-erasure admission MUST fail — D-170);
...
crevokedev = { mode: "exclude" / "compromise",
  ...
  rotation_refs: [* bytes32] }   ; set; separately committed
                                 ...
                                 ; coverage domain = the target's
                                 ;   DECRYPTABLE-WRAP zones (D-173:
...
zoneheads = { zone_id: ulid, heads: [* head] }
  ...
  ;   pends behind it — lenient advances never pend; a
  ;   late-accepted carrier re-folds the bounded consumer suffix;
  ;   a dead-lineage stage is consumed
  ;   VACUOUSLY (D-160/D-168); the staging lane
...
xferabort = { export_id: bytes16, release_op: bytes32,
  ...
  missing: [+ { rec: bytes32, ? basis: [+ bytes32] }] }
                             ...
                             ;   at (D-171) = the control-position
                             ;   op hash at terminal write — the
                             ;   committed evaluation coordinate
                             ;   (verifiable-when-held; a rebuild
                             ;   re-derives the cause over facts
                             ;   at or before it);
                             ;   per-record basis (D-140/D-148/
                             ;   D-157/D-171) = the MINIMAL op
                             ;   hash among the branch-relative
                             ;   facts sufficient at `at` (v0.5.14
                             ;   mirrors still said first-in-fold-
                             ;   order against the main rule) — a
                             ;   control
                             ;   boundary OR a tenant fact; for
                             ;   import-collision the SET carries
                             ;   the typed conjunction
                             ;   (winner, freeze_basis) — the
                             ;   terminal invalidates when EITHER
                             ;   member dies (D-169); otherwise one
                             ;   member; the vocabulary = D-149's
                             ...
                             ;   ANY member dissolves, displaces,
                             ;   or retro-quarantines
xferreopen = { export_id: bytes16, release_op: bytes32,
  ...
  basis: bytes32, invalidation: bytes32 }
                             ...
                             ;   invalidated cause MEMBER (one
                             ;   bytes32 — for a conjunctive
                             ;   collision cause, the member that
                             ;   died, D-169), invalidation =
...
mimport = { source_op: bytes32,   ; the released bundlerec this binds to
  ...
  ;   claim against a frozen owner = import-collision (its
  ;   terminal cause = the (winner, freeze_basis) conjunction,
  ;   D-169)
