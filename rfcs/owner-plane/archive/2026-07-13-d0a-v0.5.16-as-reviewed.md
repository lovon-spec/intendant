...
*v0.5.16 draft, 2026-07-13. Supersedes v0.1–v0.5.15
...
`~/agenda-rfc-archive/*d0a-v0.*-as-reviewed.md` (v0.1 through v0.5.15).
Folds the successive synthesized reviews (v0.4 through v0.5.15,
`~/owner-plane-d0a-spec-v0.5.15-synthesized-review.md` latest) with the
...
D-175/D-183 — §9.3's rule, stated here too: a held-but-displaced
named head retires its accepted predecessor, never nothing;
v0.5.15 fixed §9.3 and left this mirror on the verbatim-head
rule): a lineage may hold several live heads
...
- **Cross-zone transfer (same daemon)** is two commits in two zone
  ...
  **`release_op` critical section (D-113/D-123)**: the plane writer
  ...
  attempt** (D-119/D-126/D-178: a destination import ACCEPTED but
  not yet effect-final is an
  **unresolved** attempt and the terminal DEFERS until it
  resolves; a durable attempt still PENDING PROOF resolves
  **negative at the erasure itself** — erasure wins, D-178
  (v0.5.14's deferral let a post-erasure proof admit bytes nothing
  could any longer verify);
  ...
  **transition fold** — one EFFECTIVE terminal per interval,
  ...
  typed and monotone (D-157/D-163/D-179)**: any one branch-relative
  fact sufficient at the terminal's writing, carried as a tagged
  `factref` (an operation hash or an issuer-statement `stmt_id`) —
  ...
  outcome by KIND and total (D-163/D-179): `cutoff` and
  `scope-*` → the
  boundary/retirement op; `no-grant` with a revoked grant / fold
  `cert-revoked` → the
  ...
  control op; **`import-collision` is a FOLD outcome, never a
  ...
  intrinsic byte failures — a never-issued grant and static class
  exclusions included (no fact exists to cite, and none can
  dissolve) —
...
**One validator (D-162/D-170/D-178)**: admission is **per-record,
always** —
the leaf + path against the signed root, plus byte equality of
`rec` against the source-derived bundlerec — and **erasure wins
(D-178)**: a source erasure resolves every NOT-YET-ACCEPTED import
of that record **negative** (eligible `missing`; the pending-
attempt deferral yields to erasure — a genuine delayed import dies
with its erased source, which is what source-erased MEANS).
...
validators that disagreed after a sibling erasure; under
erasure-wins the answer never changes — A imports on its own proof
and source equality both times; B′ never imports: pre-erasure its
source equality fails, post-erasure the erasure resolved it
negative on every schedule).
...
claimants that can NEVER win release the reservation —
reject-permanent, order-losers against a frozen owner
(`import-collision`), or permanently non-revivable quarantine
(D-177 — replacing v0.5.15's undefined "frozen-out" label) —
...
before consumption is consumed VACUOUSLY — D-160/D-168/D-176); the
...
against a frozen owner is `import-collision` — a fold outcome,
never a terminal cause (D-177) — and the freeze basis dying
unfreezes the key at the FOLD level (the owner re-derives,
D-155) — D-139/
D-146/D-155/D-161/D-169/D-177);
...
log-corrupt — D-163); the monotone-cause trace (the RECORDED
cause dies while another sufficient fact holds → the journal
reopens and interval n+1 re-terminals with the survivor, both
orders + fresh fold; a stmt-kind factref cause — an issuer
statement — cited and verified — D-179); the erasure-wins
schedule (B′ pending time proof →
source erased → the erasure itself resolves the pending attempt
NEGATIVE — the later proof changes nothing, and the v0.5.14
after-erasure admission MUST fail; a genuine pending import dies
with its erased source identically — D-170/D-178);
...
crevokedev = { mode: "exclude" / "compromise",
  ...
  rotation_refs: [* bytes32] }   ; set; separately committed
                                 ...
                                 ;   domain themselves, so no
                                 ;   reference continuation exists
                                 ;   or is needed; each reference
...
factref = { kind: "op", ref: bytes32 }
  ...
  ;   issuer-statement stmt_id — journal causes and invalidations
  ;   cite EITHER domain
xferabort = { export_id: bytes16, release_op: bytes32,
  reason: "source-erased" / "reject-permanent"
        / "release-rejected",
  missing: [+ { rec: bytes32, ? basis: factref }] }
                             ...
                             ;   D-157/D-179) = ONE writer-chosen
                             ;   sufficient branch-relative fact,
                             ;   as a tagged factref (op or stmt) —
...
xferreopen = { export_id: bytes16, release_op: bytes32,
  incarnation: uint,
  basis: factref, invalidation: factref }
