...
*v0.5.5 draft (freeze candidate), 2026-07-12. Supersedes v0.1–v0.5.4
...
`~/agenda-rfc-archive/*d0a-v0.*-as-reviewed.md` (v0.1 through v0.5.4).
Folds the successive synthesized reviews (v0.4 through v0.5.4,
`~/owner-plane-d0a-spec-v0.5.4-synthesized-review.md` latest) with the
...
**Effect finality (D-94).** An operation with an external or
...
generation of its lineage below it is **closed** (terminal head
known via the `last_known` chain, or ratified by an accepted
cutoff), so no canonically earlier budget consumer can still arrive
...
escaped effect. **The generation window lives on the
...
- **T2 (witness qualification).** A receipt qualifies for an
  ...
  **service-key descriptor** (`c.service_key`, §7.1) — the descriptor,
  ...
  Admitted operations are revisited by exactly **three deterministic
  paths (D-94)**: explicit compromise cutoffs (T4, device or
  ...
  D-86), and ratify-boundary revival (§7.1 cutoff algebra, D-93).
  Each is a fold of control state — deterministic on every replica. **Lease issuers** are the same qualified witness set
...
- **Cross-zone transfer (same daemon)** is two commits in two zone
  ...
  precedence is state-derived, never order-journaled (D-98)** —
  recovery evaluates, in this order: (1) **replay keys first**: every
...
| `c.lineage_reauth` | admin (hosted: own lineage only, §7.5) | opens a new `max_generations` window on the lineage; **budgets and cutoffs persist**; body carries a REQUIRED **requester attestation** `Sign(device_sk, msg("reauth", { plane_id, lineage, max_generations, request_id, ctrl_frontier, lineage_version, repoch, gens_total }))` — bound to this operation's `request_id` (single-use, D-59), the lineage's monotonic `lineage_version` (0 at creation; never resets on the surviving branch), the current `repoch` (defeats the C3′ branch-cut path that could rewind the derived count — D-87), **and `gens_total`** (the lineage's lifetime accepted `w.gen` count, summed across zones — monotone with consumption, so the assent is to the *current exhaustion state* and a banked attestation dies on the next generation, D-95); admission requires equality with all three values at this operation's control position; the admin-arm header alone cannot prove "own lineage" | reject-permanent |
...
- **ratify** (`c.cutoff`; renewal `history_cutoffs`): grows the
  ...
  operations (cascading, deterministic re-derivation).
- **revoke** (`c.revoke_device` cutoffs; `c.revoke_grant.cutoff`;
  ...
  **immutable once accepted** — never composed, never widened; a
  ...
  alone; the equality pins below scope it).
...
lifecycle**: a cutoff naming a tenant Head not yet held is
**pending-dependency** until the exact bytes arrive at that chain
position; a later arrival at the same coordinate with a different
...
- **C3′ (recovery resolution, deterministic).**
  ...
  **Storage adoption (D-97)**: recovery whose base precedes an
  accepted `c.kek_rotate` either lists it in `adopted_rotations` —
  its KEK epoch, wraps, and recipient commitment survive as adopted
  state, keeping activated storage readable — or leaves it cut: a
  replica that already **activated** that rotation (Fenced; old KEK
  possibly destroyed) transitions to **storage-quarantine** (its
  post-Fence ciphertext is under an epoch the surviving control
  state no longer carries). The choice is the owner's, explicit and
...
admit_ctrl(op, control_state):
  ...
  prec   C3′ precedence-field validity (base/epoch/repoch/commitment
         — from the now-validated body); ONLY THEN placement C1–C5 +
...
| **pending-dependency** (held until the referenced input arrives; fence-hardening per §7.1 `c.checkpoint` — deadline/lease/causal only, D-77) | causal-missing, cert-superseded (awaiting renewal chain), policy-missing, deadline-unreceipted, lease-missing (§9.1), epoch-unopened (awaiting the epoch-opening control op — D-78), issuer-gap (awaiting the missing chain link — D-87) |
...
`max_bytes` — a deterministic, content-independent surcharge
...
recovery storage adoption both branches (adopted rotation survives;
unadopted activated rotation → storage-quarantine — D-97);
checkpoint chain
...
`gens_total` staleness (a banked attestation dies on the next
generation — D-95); dropped-witness forever-pending lane (D-88)); 10 lineage/budget (generation
...
open unknown-gap defers until the cutoff — D-94); the grant-epoch
...
no writer-asserted size, D-98); frozen-stamp
PendingXfer recovery; state-derived terminal precedence
...
RewrapDone — D-97); the KEM-renewal drain precondition; renewal
wrap supersession by (zone, epoch, device);
...
| D-80 | Checkpoint embedded and chained: `c.checkpoint`'s body IS the versioned object (op hash = identity; control log = carriage; `H_ckpt` dead); `prev_checkpoint` chains; `retired` = since-predecessor deltas (≤ 256) removed exactly by the fold; `proof_positions` = currently qualified issuers (≤ 64); the live Frontier holds accepted heads only; `c.cutoff` carries a bounded cutoff set (multi-head unknown-gap retirement) — covers paged + explicit fences + the E6 `v` fix by D-88 | Ratified, owner, 2026-07-12 |
...
cenrollrenew = { cert: cert,                ; cert.renews REQUIRED
  ...
  ? wraps: [* kekwrap] }
      ...
      ;   per accepted epoch of every held zone, each
      ...
      ;   unchanged (old wraps stand) — D-89. KEM-rotating renewal
      ;   REQUIRES accepted memberships <= 128 (drain queued
      ;   rotations first — hard precondition, D-97); grants and
      ;   lineage NEVER appear (D-84)
...
clineagereauth = { lineage: bytes16, max_generations: uint,
  ...
  ;   repoch, gens_total })) — single-use (request_id consumed,
  ;   D-59) and fresh (D-82/D-87/D-95: lineage_version = accepted
  ;   reauth count, 0 at creation, monotone; repoch defeats C3′
  ;   resurrection; gens_total = the lineage's lifetime accepted
  ;   w.gen count — assent binds the current exhaustion state)
...
zoneheads = { zone_id: ulid, heads: [* head] }
  ...
  ;   repoch, gens_total })) — single-use (D-59), fresh
  ;   (D-82/D-87/D-95: lineage_version + repoch + gens_total
  ;   equality); live_heads is CARRIED in the body (above) and
  ;   admission requires each named zone's live-head set to equal
  ;   the lineage's current one — assent invalidates the moment the
  ;   lineage writes past it (banked rollback authority dies)
...
crecovsucc = { base: { seq: uint, op: bytes32 }, epoch: uint,
  ...
  ; set (E7), keyed by zone_id (D-97): rotations on the CUT branch
  ;   whose KEK epochs/wraps survive as adopted state — a replica
  ;   that activated an unadopted cut rotation (old KEK destroyed)
  ;   storage-quarantines; the owner chooses, explicitly, per zone
...
fenceframe = { kek_epoch: uint, rotation_op: bytes32,
  ...
  ; recipients_hash = H_recips(canonical sorted
  ;   [* { device: bytes16, kem_key: bytes32 }]) — the closed
  ;   intended recipient set; control_frontier = the control position
  ;   whose accepted wrap-adds are IN the intent
