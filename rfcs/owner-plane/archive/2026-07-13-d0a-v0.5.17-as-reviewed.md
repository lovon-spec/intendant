...
*v0.5.17 draft, 2026-07-13. Supersedes v0.1–v0.5.16
...
`~/agenda-rfc-archive/*d0a-v0.*-as-reviewed.md` (v0.1 through v0.5.16).
Folds the successive synthesized reviews (v0.4 through v0.5.16,
`~/owner-plane-d0a-spec-v0.5.16-synthesized-review.md` latest) with the
...
- Renewal: `c.enroll` with the same `device_id` + `renews`, carrying
  ...
  **legitimately omitted** from coverage (its last grant already
  ...
  ending operation's control position — never
...
- **Cross-zone transfer (same daemon)** is two commits in two zone
  ...
  **`release_op` critical section (D-113/D-123)**: the plane writer
  ...
  attempt** (D-119/D-126/D-184: a destination import COMMITTED —
  its ItemCommit durable in the zone log — but
  not yet effect-final is an
  **unresolved** attempt and the terminal DEFERS until it
  resolves; an attempt NOT YET COMMITTED at the erasure's
  serialization point — proof-pending included — resolves
  **`source-erased` at the erasure itself** (reject-permanent,
  D-184: the commit is the boundary; v0.5.16 keyed on acceptance,
  which post-erasure had no derivable evidence, and v0.5.14's
  deferral let a post-erasure proof admit bytes nothing
  ...
  **transition fold** — one EFFECTIVE terminal per interval,
  ...
  intrinsic byte failures, never-issued grants, static class
  exclusions,
...
|---|---|
| **reject-permanent** (never re-enters within the branch — **the split is by the rejecting fact, D-149**: intrinsic byte failures (malformed, oversized, depth, non-canonical, sig-invalid, body-hash, key-malformed, proof-arm, gen-first-op, source-erased) never re-enter ANYWHERE; rejections whose fact is a control operation (no-grant, scope-*, cert-revoked-in-fold, provenance/hosted-ceiling, import-collision, control-relative body-invariant instances) re-evaluate under the C3′ total re-fold when that fact dissolves — D-138) | malformed, oversized, depth, non-canonical, sig-invalid, body-hash, body-invariant, key-malformed, proof-arm, no-grant, scope-*, class-*, provenance-ceiling, hosted-ceiling, no-flow, gen-first-op, request-fork (surfaced as fork evidence), source-erased (an import whose source was erased before its commit — intrinsic, the source is cryptographically gone; D-184/D-189), cert-superseded (the PROVEN-incompatible context: a grant issued after the certificate ceased effective — D-187; the awaiting-renewal-chain context stays pending-dependency), import-collision (a claim against a FROZEN owner — the only claim that can never win; an order-loser against an UNFROZEN owner is displaced instead, quarantine-reproposal, whichever arrived first — arrival-invariant, D-161) |
| **pending-dependency** (held until the referenced input arrives; fence-hardening per §7.1 `c.checkpoint` — deadline/lease/causal only, D-77) | causal-missing, cert-superseded (awaiting renewal chain — the PROVEN-incompatible context is reject-permanent, D-187), policy-missing, deadline-unreceipted, lease-missing (§9.1), epoch-unopened (awaiting the epoch-opening control op — D-78), issuer-gap (awaiting the missing chain link — D-87), ref-unresolved (awaiting a referenced Head / rotation / adoption target / an applicable staged carrier — D-176 / an unheld journal citation, which reserves its interval — D-185 / a provisional import claim's freeze — D-189; D-102) |
...
`rec` against the source-derived bundlerec — and **the COMMIT is
the erasure boundary (D-184)**: a source erasure resolves every
import of that record **without a committed ItemCommit at the
erasure's serialization point** to outcome `source-erased`
(reject-permanent; eligible `missing`) — the zone log's committed
ItemCommit is authenticated durable bytes written by the plane
writer only after source equality passed, and the `release_op`
critical section serializes it against the erasure, so the order
is IN THE LOG: both schedules and a cold rebuild converge by
replaying it (v0.5.16 keyed survival on ACCEPTANCE — an
acceptance-history fact with no derivable post-erasure evidence,
the class this document removes everywhere; a proof-pending
attempt is uncommitted and dies at the erasure identically).
...
— D-182) + the negation-residual acceptance (−P enrolls — exact-
...
whose basis factref is unheld pends `ref-unresolved` and RESERVES
its interval — no second terminal lands; the re-fold applies it
when the citation arrives, both orders + fresh fold — D-185); the monotone-cause trace (the RECORDED
...
D-179/D-185); the commit-boundary
schedules (B′ pending time proof →
source erased → the uncommitted attempt resolves `source-erased`
at the erasure — the later proof changes nothing, and the v0.5.14
after-erasure admission MUST fail; proof → COMMIT → erase → cold
rebuild — the committed ItemCommit replays from the log and the
import stands; a genuine pending import dies
with its erased source identically; both orders + fresh rebuild —
D-170/D-178/D-184);
...
mimport = { source_op: bytes32,   ; the released bundlerec this binds to
  ...
  proof: [* bytes32],               ; Merkle siblings, <= 7 (E8) —
                                    ...
                                    ;   rule — leaf+path PLUS
                                    ;   source equality, with the
                                    ;   committed ItemCommit as the
                                    ;   erasure boundary (D-184);
                                    ;   the proof alone NEVER
                                    ;   admits post-erasure)
  provenance: {
