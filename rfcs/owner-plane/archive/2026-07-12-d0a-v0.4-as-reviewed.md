...
*v0.4 draft (freeze candidate), 2026-07-12. Supersedes v0.1–v0.3
...
`~/agenda-rfc-archive/*d0a-v0.{1,2,3}-as-reviewed.md`. Folds the v0.3
synthesized review
(`~/owner-plane-d0a-spec-v0.3-synthesized-review.md`) with the owner's
...
- **E8 (hard caps — deterministic parsing).**
  ...
  | rotation `erase_manifest` | ≤ 4096 |
  | revocation `rotation_refs` | ≤ 16 |
...
- Renewal: `c.enroll` with the same `device_id` + `renews`, carrying
  ...
  operations at or before the cutoffs remain valid **with no receipt
  requirement** (this is what keeps receipt-free solo history alive
  across renewal); the old key authors nothing new. Grants bind
...
- **T2 (self-exclusion by device).** A receipt qualifies for an
  ...
  **different `device_id`** than the operation's signer — a renewed
  key cannot receipt its own old-key operations. Zone policy
  `time_witnesses` lists **stable `device_id`s** (never renewable cert
  hashes) and/or the literal `"connect"`; `connect_service_key` is
  REQUIRED exactly when `"connect"` is listed or `accept_connect_time`
  is true.
- **T3 (anti-replay and issuer lifecycle).** `issuer_seq` is strictly
  monotone per `(device_id, signing key)` scope, persisted in the
  issuer's own durable store; **key renewal restarts the scope at 1**
  (a new key is a new scope) — which is also the recovery path after a
  snapshot rollback: an issuer that cannot trust its own counter
  renews its certificate. Duplicate `(scope, issuer_seq)` with
...
6 Tombstoned         EraseTombstone { v:1, item_addr, erase_op: bytes32,
                     ...
                     the rotation op's erase_manifest
```
...
- **Txn rules**: all records belong to this zone log; ItemCommits come
  ...
  (claim, then its self-accept judgment).
...
- **Cross-zone transfer (same daemon)** is two commits in two zone
  ...
  source zone commits one Txn = [release ItemCommit,
  PendingXfer { export_id, dest_zone }]; after the destination's
  `m.import.claim` commits, the source commits `XferDone
  { export_id }`. Recovery finding a PendingXfer with no matching
  XferDone re-runs the import (idempotent by
  `(from_plane, export_id)`). **A PendingXfer clears only on its
  matching XferDone** — never on OutboxMark, which cannot identify a
  transfer.
...
|---|---|---|---|
| `c.genesis` (seq 1 only) | genesis | installs descriptor; admin epoch 1; repoch 0; creates first cert, lineage, `private` zone (epoch-1 KEK + wrap to the first device), **exactly one `home` space** (space_class `personal`, class_minimum `private` — D-39/D-40 — status_policy `workflow-v1`) **and exactly one `audit` space** (space_class `audit`, class_minimum `private`, status_policy `owner-v1`), the B.1 ZonePolicy template instantiated, the first device grant (finite budget REQUIRED — budgets posture; hosted planes: the §7.5 safe verb set; trusted: full verbs), and the **audit grant** (`audit.write` only, audit space, the device's service lineage) | reject-permanent |
| `c.enroll` | admin | cert (new / `renews` **with required `history_cutoffs`**, §4.2); grants[]; lineage create-or-reauthorize (**non-resetting**; at most one live lineage per device — a second while one lives rejects); `WrapAdd`s for granted zones | reject-permanent |
| `c.revoke_device` | admin | `mode: "exclude" / "compromise"`; cert revoked; grants revoked; lineage cutoff at named head; rotations **by reference** (`rotation_refs` naming separately committed `c.kek_rotate` ops — embedded rotations would overflow the control cap): the revocation is pending-dependency until every referenced rotation is accepted, then takes effect atomically; `compromise` also retro-disqualifies the device's receipts past the cutoff (T4) | pending-dependency → reject-permanent |
...
| `c.lineage_reauth` | admin (hosted: own lineage only, §7.5) | opens a new `max_generations` window on the lineage; **budgets and cutoffs persist**; body carries a REQUIRED **requester attestation** `Sign(device_sk, msg("reauth", { lineage, max_generations }))` proving the named device asked (the admin-arm header alone cannot prove "own lineage") | reject-permanent |
...
(existing certificates are grandfathered). Where the deadline field is
absent (D-28):
...
- **`w.gen`** (registry §11.1): the charged operation opening
  ...
  write-capable grant, charged 1 op against it. **The generation
  ...
  `c.lineage_reauth`. Frontier effect: accepting
  `w.gen(g, last_known = head)` retires every head of the lineage with
  gen < g (D-33); with `last_known = "unknown"`, prior heads persist
  until an admin `c.cutoff` ratifies the abandonment. Operations
  arriving for a retired generation: quarantine-reproposal.
...
Budget accounting resets per epoch (§4.3).
...
| `m.unpin` (safe) | pin.safe | munpin | direct-human evidence; target_pin authored by the **same device** (relation self) — hosted writers can undo their own pins | pin revoked | 1 op |
...
| `m.audit` | **audit.write** (the genesis audit grant, §7.1 — grantable to service actors only) | maudit | actor.kind == service; attested_by == the writing device's own cert; space == audit; works on **any device class** (a zero-daemon browser audits its own sensitive reads). **Failure mode**: a sensitive-space read whose audit row cannot durably commit **fails closed** (the audit append is part of the read transaction); audit-space reads are themselves audited — writing an audit row is a write, so recursion terminates | audit row appended | 1 op |
...
input). Taint depth 1. Plane refs are **verified**: their taint is
`evidence_effective_floor` — the referenced claim's `effective` class
evaluated at the projection's stamped frontier — max'd with the
recorded `class_floor`. **A dangling or unresolvable reference of
either namespace taints as `sensitive`** (owner ruling D-36 — a
claimant's recorded floor is not verification). Resolvable external
refs contribute their recorded `class_floor`, stated as a trust
boundary (external content has no verifiable class). Locator
...
- Projections stamp `{data frontier, control frontier, reducer
  ...
  (`status(c, F, P, as_of)` consumes `as_of` through exactly these
...
`source_op`, and its `statement`/`kind` MUST equal that record's
(destination-verified against the received bundle bytes); one release
of up to 128 records maps to up to 128 imports.
...
`class_floor`; replay keys on `(from_plane, export_id)` and is
idempotent (`duplicate`). Same-plane cross-zone re-encrypts as new
items in the destination zone (§6.1 PendingXfer/XferDone recovery).
Release `expiry_deadline_ms` follows §9.1 acceptance-deadline
semantics for the import operation.
...
keystream is one byte stream, draws taken in the vector's documented
input order. Where browser WebCrypto cannot inject randomness (P-256
...
fail-closed; `valid_from` / erased-status /
...
zonepolicy = { v: 1, zone_id: ulid, strictness: "strict" / "lenient",
  ...
  ? time_witnesses: [+ bytes16 / "connect"],   ; stable device_ids
  ? connect_service_key: bytes32 }
  ; connect_service_key REQUIRED iff accept_connect_time or
  ;   "connect" in time_witnesses
...
crevokedev = { mode: "exclude" / "compromise",
  revocation_id: bytes16, revoke_grants: [* bytes16],
  cutoff: { lineage: bytes16, accepted_through: head },
  rotation_refs: [* bytes32] }   ; op hashes of separately committed
                                 ; c.kek_rotate ops (<= 16, E8)
...
clineagereauth = { lineage: bytes16, max_generations: uint,
  requester: { device_cert: bytes32, sig: bstr } }
  ; sig = Sign(device_sk, msg("reauth", { lineage, max_generations }))
...
rewrapdone = { kek_epoch: uint, rotation_op: bytes32, count: uint,
  survivors: bytes32 }  ; H_survivors(sorted [{item_addr, wrap_hash}])
...
mimport = { source_op: bytes32,   ; the released bundlerec this binds to
  ...
  ? labels: [* text] }
  ; statement/kind MUST equal the bound bundlerec (S11.8); replay keys
  ; on (provenance.import.from_plane, export_id)
...
rule = { verdict: verdictname, kinds: [+ kind] / "*",
  ...
  ; relation REQUIRED: "self" = judging actor is the target claim's
  ;   authoring actor+session; "author" = the judging DEVICE authored
  ;   the target; "any" = no relational constraint
```
...
lenient_epochs: 0, accept_connect_time: false,
deadline_fallback: "budgets", require_cert_deadlines: false }` — the
