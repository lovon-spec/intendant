...
*v0.5.3 draft (freeze candidate), 2026-07-12. Supersedes v0.1–v0.5.2
...
- **E7 (sets and sort keys).** Every array documented as a *set* is
  ...
  `proof_positions` → tagged issuer; checkpoint `retired` →
  `(lineage, gen)`; `rotation_refs` → op hash;
...
- **E8 (hard caps — deterministic parsing).**
  ...
  | cutoff sets (revocation / epoch-advance closure / `c.cutoff`) | ≤ 64 each (revocation continues via `c.revoke_zones`) |
...
row's `bundle_size`, D-85); each
operation cites exactly one grant; budgets account per
`(grant_id, lineage)` per **budget window** — the span between admin
`c.cap_epoch_bump` operations — and reset only on such a bump
(deliberate; a `c.zone_policy` epoch advance re-arms nothing, D-79). **The generation window lives on the
...
`c.checkpoint` covers it — the frontier drops **exactly the
...
- **T2 (witness qualification).** A receipt qualifies for an
  ...
  **service-key descriptor** (`c.service_key`, §7.1) — the descriptor,
  ...
  Admitted operations are revisited **only** by explicit compromise
  cutoffs (T4, device or service): at the cutoff's fold position, an
  operation whose acceptance rested solely on now-disqualified
  proofs transitions to quarantine-reproposal — deterministic,
  because the cutoff is a control operation every replica folds
  identically. **Lease issuers** are the same qualified witness set
...
- **T3 (anti-replay and issuer lifecycle).** `issuer_seq` is
  ...
  **one counter per scope, shared by receipts and leases alike,
  ...
  durable store. Density is load-bearing (D-77/D-85): a compromise cutoff's
  `through = N` names an **observed contiguous feed head** — the
  cutoff's author held statements 1..N (`0` = the empty feed) — so no
  unused position ≤ N exists honestly; a **new** (non-colliding)
  statement at seq ≤ N arriving later is outcome `issuer-gap`
  (backfill mint, or a lying cutoff — either way that issuer's feed
  freezes pending owner review); byte-colliding mints quarantine as
  `issuer-fork`; repeated cutoffs for one scope merge at the minimum; **key renewal restarts the scope at 1** (a new
...
- **I3 (active wrapper).** The served wrapper is the one whose
  ...
  *authorizes* a rotation; its Fence *activates* it (D-81); wrappers
  of retired (pre-Fence) epochs are not served.
...
2 Fenced             zone-log Fence record — persists { kek_epoch,
                     rotation_op, fence_frontier } (D-77; equal to
...
**Serialization (D-81)**: a zone's rotations run strictly in
control-chain order — rotation N+1's Fence may be written only after
rotation N's `KekDestroyed` frame is durable (a crash re-enters the
queue at the recorded state; control admission stays portable, since
epoch consecutiveness already orders acceptances — the queue is a
local storage invariant). **Memberships above 128 (D-81)**: the
rotation's bounded wrap set temporarily excludes the remainder;
current-epoch `c.wrap_add`s re-admit each intended non-target holder,
and `RewrapComplete` waits until every intended survivor holds a
current-epoch wrap (family-13 vector). **Last-holder precondition
(D-81)**: an exclusion-shaped rotation must retain ≥ 1 recipient —
excluding a zone's final holder is not expressible as exclusion
(zone abandonment is a distinct ceremony, out of D0-A scope).
...
0x13 Fence       { kek_epoch: uint, rotation_op: bytes32,
                   fence_frontier: bytes32 }   # D-77: the expected-
                                               # membership point
                                               # survives a crash
                                               # before RewrapDone
...
- **Cross-zone transfer (same daemon)** is two commits in two zone
  ...
  (sorted by op), re-runs only the missing imports, and writes the
  terminal record; permanently failed records (erased source,
  destination `reject-permanent`) do not stop the remainder —
  recovery completes every completable import, then one `XferAbort`
  collects ALL missing source_ops. **Terminal states
...
| `c.zone_policy` | admin | install full ZonePolicy (versioned; explicit — policy NEVER changes implicitly, incl. on witness arrival; owner ruling D-28); **acceptance advances the zone's capability epoch by 1 (D-69)** — operations signed at older epochs keep their old policy anchor (§9.4); `deadline_fallback = "fail-closed"` REQUIRES `require_cert_deadlines = true` (cross-field, D-76); **closure cutoffs (D-78)**: MAY carry `cutoffs` closing old-epoch writing per lineage — REQUIRED covering every live lineage when the policy in force at acceptance is `strict` | reject-permanent |
...
| `c.grant` / `c.revoke_grant` | admin | issue / revoke capability; **revoking an op-authoring grant carries a REQUIRED `cutoff` (D-78)** — operations at or before it stand, beyond it quarantine (`cutoff`); read-only revocation is edge-effective (no chain impact) | reject-permanent |
| `c.cap_epoch_bump` | admin | zone capability epoch +1 — body `new_epoch` MUST equal the zone's current capability epoch + 1 (`body-invariant`; §9.4 consecutiveness, consumed at validation); opens a new budget window (D-79); closure `cutoffs` per the D-78 rule (REQUIRED covering every live lineage under `strict`) | reject-permanent |
| `c.lineage_reauth` | admin (hosted: own lineage only, §7.5) | opens a new `max_generations` window on the lineage; **budgets and cutoffs persist**; body carries a REQUIRED **requester attestation** `Sign(device_sk, msg("reauth", { plane_id, lineage, max_generations, request_id, ctrl_frontier, lineage_version }))` — bound to this operation's `request_id` (single-use, D-59) **and to the lineage's monotonic `lineage_version`** (D-82, superseding D-72's resetting `window_state`: the count of accepted `c.lineage_reauth` operations for this lineage — never resets, so a banked attestation can never become valid again; zone-free, so the peer's per-zone ambiguity dissolves); admission requires equality with the version at this operation's control position; the admin-arm header alone cannot prove "own lineage" | reject-permanent |
...
| `c.cutoff` | admin (hosted: own lineage only, §7.5) | a **set** of zone-qualified cutoffs (≤ 64, keyed `(zone_id, lineage)` — one operation retires several unknown-gap heads, D-80); MAY carry a **requester attestation** `Sign(device_sk, msg("cutoffreq", { plane_id, cutoffs, request_id, ctrl_frontier, lineage_version }))` — REQUIRED on hosted planes (D-54, every cutoff naming the requester's own lineage); `requester.device_cert` resolves to a certificate whose `device_id` owns that lineage and the signature verifies under that certificate's key (D-77); bound to this operation's `request_id` (single-use, D-59) and the lineage's monotonic `lineage_version` (D-82) | reject-permanent |
| `c.abandon_writer` | admin | lineage abandoned at head | reject-permanent |
| `c.checkpoint` | admin | **zone-scoped (D-62), embedded (D-80)**: the body IS the versioned checkpoint object (App A.2 `checkpointobj`) — the operation hash is its identity and the control log its carriage (no `H_ckpt` indirection). `prev_checkpoint` chains the zone's checkpoints (`"genesis"` first); `retired` = heads retired **since the predecessor** (≤ 256), each validated per D-76 (live at the predecessor, covered by `covers`, incorporation-eligible) — the fold transition removes exactly `retired` from the live Frontier; `proof_positions` = the **currently qualified** issuers only (≤ 64; closed feeds live behind their existing cutoffs, never re-asserted). **The fence IS `covers`**: an operation's position is its signed `(lineage, gen, seq)` — covered iff ≤ that chain's head in `covers` (a pending operation occupies its signed coordinates; the live Frontier holds accepted heads only). **Hardening (narrowed, D-77)**: a fence covering a pending operation's position AND every qualified witness feed hardens `deadline-unreceipted`/`lease-missing` to quarantine; `causal-missing` hardens when the fence covers the missing predecessor's coordinates and it is absent; `cert-superseded`/`policy-missing` never fence-harden (control-chain dependencies stay pending, owner-visible) | reject-permanent |
...
- **Grant slack**: `op.capability_epoch − grant.capability_epoch ≤
  grant_epoch_slack` (absent = unbounded) — both operands signed,
  fully portable; violation → `capability-epoch`.
...
admit(op, control_state, receipts):
  ...
  cert   resolve signer cert by device; status; class table §7.6;
         signer_alg == cert.sig_alg ∧
         signer_key_id == H_key({alg, pk}) (D-85)
                                               → cert outcomes
...
their listed order. Control operations run the same ordered stages
with arm admissibility (§7.2) in place of grant scope at the proof
stage (D-85).
...
| **pending-dependency** (held until the referenced input arrives; fence-hardening per §7.1 `c.checkpoint` — deadline/lease/causal only, D-77) | causal-missing, cert-superseded (awaiting renewal chain), policy-missing, deadline-unreceipted, lease-missing (§9.1), epoch-unopened (awaiting the epoch-opening control op — D-78) |
| **quarantine-reproposal** (RFC §4.5 lane) | capability-epoch, cutoff, lease-stale, budget, lineage-gen |
| **duplicate-idempotent** | duplicate (byte-identical) |
| **freeze-control** | ctrl-fork (C2), recovery-competition (same-repoch, §7.4) |
| **freeze-writer** | fork (tenant writer), issuer-fork / issuer-gap (that issuer's feed) |
...
across different flows. **Charge (D-75/D-85)**: the release charges its canonical operation
bytes plus the signed `bundle_size` against the grant's `max_bytes`;
`bundle_size` MUST equal the canonical bundle's byte length
(validated at admission while the sources are readable, replayed
as-signed thereafter — the charge survives source erasure).
...
(D-85)**: each family defines a closed `case_kind` vocabulary, and
each case_kind pins its exact `inputs` and `expected.result` field
sets — object inputs are lowercase hex of canonical CBOR bytes,
results are canonical encodings or closed-enum strings; a harness
never invents family semantics. `draw_order` names are unique within
...
the pinned 314 B/132 B worst-width figures and the 128+128 rotation
fit); 2 domains/key-ids;
3 signatures (low-S);
...
(now provable under D-78's portable currency); strict-zone closure
(advance without covering cutoffs rejects; closed-lineage old-epoch
op quarantines); grant-revocation cutoff; renewal-mints-no-grants
negative (D-84); one-live-compound-per-revocation_id; derived grant
revocation; checkpoint chain (prev linkage, since-predecessor
retired, embedded-object fold transition); genesis grant
completeness (the 15-verb set, sensitive ceilings, inert
raise/declassify, no deadline/lease); class table incl. mobile-attested vs mobile; renewal
...
equivalence); attestation `lineage_version` freshness (a banked
attestation can never re-validate — D-82); dense-prefix cutoffs
(`issuer-gap` on a below-cutoff new statement; repeated-cutoff
min-merge; device compromise cutoffs cover leases); dense
`issuer_seq` backfill denial); 10 lineage/budget (generation
non-reset; window exhaustion; cutoff across generations; epoch bump
reset — budget windows reset on `c.cap_epoch_bump` only, a
`c.zone_policy` advance re-arms nothing (D-79); portable currency
...
matching; signed `bundle_size` accounting (mismatch rejects; replay
after source erasure); frozen-stamp PendingXfer recovery; export_id
single-use; XferAbort terminal (erased source);
...
an epoch-1-committed item (D-73); rotation-queue serialization +
Fence activation (I3 serves the last-Fenced epoch, D-81); the staged
>128-membership ceremony;
...
| D-69 | Proof-policy anchor = the operation's **signed `capability_epoch`**: `policy(e)` = the ZonePolicy in force when epoch `e` opened; `c.zone_policy` acceptance advances the epoch (a policy change is an epoch event); compromise cutoffs are the sole revisit of admitted operations (→ quarantine-reproposal at the cutoff's fold position) — currency semantics made fully portable by D-78 (the fold-relative staleness rules are void) | Ratified, owner, 2026-07-12 |
...
| D-75 | Transfer authority: release deadline ≤ the matching flow's deadline; flow matching is existential and whole (one entry admits every axis); release charges op + bundle bytes against `max_bytes`; bundles are never persisted or framed (re-derived); plane-import header equality covers all three endpoint coordinates; egress proof = source-zone receipt on the release's own `item_addr`; bundle/bundlerec normative in App A.5 — charge made replayable by D-85 (signed `bundle_size`) | Ratified, owner, 2026-07-12 |
...
| D-80 | Checkpoint embedded and chained: `c.checkpoint`'s body IS the versioned object (op hash = identity; control log = carriage; `H_ckpt` dead); `prev_checkpoint` chains; `retired` = since-predecessor deltas (≤ 256) removed exactly by the fold; `proof_positions` = currently qualified issuers (≤ 64); the live Frontier holds accepted heads only; `c.cutoff` carries a bounded cutoff set (multi-head unknown-gap retirement) | Ratified, owner, 2026-07-12 |
| D-81 | Fence activates: control acceptance authorizes a rotation, its durable Fence activates it (I3 serves the last-Fenced epoch); `kek_epoch` in Fence/RewrapDone = the new epoch; per-zone rotations serialize in control order (N+1's Fence only after N's KekDestroyed); >128 memberships = bounded wrap set + `c.wrap_add` re-admission before RewrapComplete; exclusion-shaped rotations retain ≥ 1 recipient (last-holder exclusion is not expressible) | Ratified, owner, 2026-07-12 |
| D-82 | Requester freshness = the monotonic `lineage_version` (count of accepted `c.lineage_reauth` for the lineage; never resets, zone-free) bound into both attestations — supersedes D-72's resetting `window_state`, which could re-validate banked attestations and was zone-ambiguous | Ratified, owner, 2026-07-12 |
...
checkpointobj = { v: 1, zone_id: ulid,
...
crevokedev = { mode: "exclude" / "compromise",
  ...
  ? receipt_cutoffs: [+ { key_id: bytes32, through: uint }],
                                 ...
                                 ;   through = observed contiguous feed
                                 ;   head (0 = empty — D-85); covers
                                 ;   leases too
...
clineagereauth = { lineage: bytes16, max_generations: uint,
  ...
  ;   — single-use (request_id consumed, D-59) and fresh
  ;   (lineage_version equality, D-82: the count of accepted
  ;   c.lineage_reauth ops for this lineage — monotone, zone-free,
  ;   control-chain-derived; never resets)
...
ccutoff = { cutoffs: [+ zonecutoff],
  ...
  ;   request_id, ctrl_frontier, lineage_version })) — single-use
  ;   (D-59), fresh (lineage_version equality, D-82)
...
fenceframe = { kek_epoch: uint, rotation_op: bytes32,
  fence_frontier: bytes32 }       ; 0x13 (D-77)
...
mexportrel = { export_id: bytes16,
  ...
  bundle_size: uint,        ; canonical bundle byte length (D-85):
                            ;   validated while sources are readable,
                            ;   replayed as-signed after erasure
