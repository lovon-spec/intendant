...
*v0.5.4 draft (freeze candidate), 2026-07-12. Supersedes v0.1–v0.5.3
...
`~/agenda-rfc-archive/*d0a-v0.*-as-reviewed.md` (v0.1 through v0.5.3).
Folds the successive synthesized reviews (v0.4 through v0.5.3,
`~/owner-plane-d0a-spec-v0.5.3-synthesized-review.md` latest) with the
...
`reauth`, `cutoffreq`, `stmtid`, `survivors`. Derivation domains:
...
operations move to quarantine-reproposal (surfaced; bounded to one
lineage; occurs only under real unknown-gap concurrency). Every
replica therefore converges on the same accepted set regardless of
arrival order. **The generation window lives on the
...
- **T3 (anti-replay and issuer lifecycle).** `issuer_seq` is
  ...
  **one counter per scope, shared by receipts and leases alike,
  ...
  `through = 0`) — a backfill mint below the cutoff cannot chain
  (the true successor already names the true `stmt_id`), so it
  resolves `issuer-fork` identically on every replica; honestly
  delayed statements fill their links and verify. Repeated cutoffs
  for one scope merge at the minimum; **key renewal restarts the scope at 1** (a new
...
accepted, unretired epoch — a later rotation's acceptance never
strands them) re-admit each intended non-target holder. **Intent
closes at the Fence (D-89)**: the intended recipient set =
the rotation's wraps ∪ wrap-adds for its epoch accepted before the
Fence frame is written; `RewrapComplete` waits until every intended
survivor holds that epoch's wrap (family-13 vector). **Last-holder
...
- **Cross-zone transfer (same daemon)** is two commits in two zone
  ...
  triggers diverge (D-90)**: a destination `reject-permanent` for one
  record leaves the others completable (the bundle is still
  derivable) — recovery finishes them, then writes `XferAbort`
  carrying the rejected records; a **source erasure makes the flat
  bundle underivable** (the erased record's statement is gone, so no
  record's membership in `content_digest` can be re-proven) —
  recovery then writes `XferAbort` at once, `missing` = every
  un-imported source_op, `reason = "source-erased"` (erasure wins;
  already-committed imports stand). `reason` = the FIRST terminal
...
| `c.enroll` | admin | a **CDDL union of two shapes discriminated by `cert.renews`** (D-89 — v0.5.3's single shape made the D-84 renewal unencodable). **New device** (`cenrollnew`): cert + `grants[]` + lineage + wraps for granted zones. **Renewal** (`cenrollrenew`): cert + `history_cutoffs` (≤ 64; zones beyond covered by pre-established standalone `c.cutoff`s — union coverage validated) + replacement wraps **iff the KEM key rotated** (one per accepted, unretired epoch of every already-held zone, superseding by `(zone, epoch, device)` key; no new-zone acquisition; absent when the KEM key is unchanged) — `grants[]` and `lineage` never appear: renewal mints no authority (D-84). **The renewed certificate's `class` MUST equal the predecessor's** (a class change is a new enrollment ceremony, never a renewal — D-89); at most one live lineage per device — a second while one lives rejects; **every `grants[]` entry and every wrap targets the enrolled/renewed certificate's `device_id` (D-76)** | reject-permanent |
...
| `c.wrap_add` | admin | adds a KekWrap for an enrolled device at the epoch it names — valid for **any accepted, unretired epoch** (D-89: a queued rotation's staging survives a later rotation's acceptance); a wrap for an existing `(zone, epoch, device)` logical key **supersedes** the prior wrap | reject-permanent |
...
| `c.grant` / `c.revoke_grant` | admin | issue / revoke capability; issuance to a device whose `revocation_id` is revoked **rejects** (`body-invariant`, D-92); **revoking an op-authoring grant carries a REQUIRED `cutoff` (D-78)** — operations at or before it stand, beyond it quarantine (`cutoff`); read-only revocation is edge-effective (no chain impact) | reject-permanent |
...
| `c.lineage_reauth` | admin (hosted: own lineage only, §7.5) | opens a new `max_generations` window on the lineage; **budgets and cutoffs persist**; body carries a REQUIRED **requester attestation** `Sign(device_sk, msg("reauth", { plane_id, lineage, max_generations, request_id, ctrl_frontier, lineage_version }))` — bound to this operation's `request_id` (single-use, D-59) **and to the lineage's monotonic `lineage_version` plus the current `repoch`** (D-82/D-87: the version — 0 at creation — never resets on the surviving branch, and repoch equality defeats the C3′ branch-cut path that could rewind the derived count; a banked attestation can never become valid again); admission requires equality with both values at this operation's control position; the admin-arm header alone cannot prove "own lineage" | reject-permanent |
...
| `c.cutoff` | admin (hosted: own lineage only, §7.5) | a **set** of zone-qualified cutoffs (≤ 64, keyed `(zone_id, lineage)` — one operation retires several unknown-gap heads, D-80); MAY carry a **requester attestation** `Sign(device_sk, msg("cutoffreq", { plane_id, cutoffs, request_id, ctrl_frontier, lineage_version }))` — REQUIRED on hosted planes (D-54, every cutoff naming the requester's own lineage); `requester.device_cert` resolves to a certificate whose `device_id` owns that lineage and the signature verifies under that certificate's key (D-77); bound to this operation's `request_id` (single-use, D-59), the lineage's monotonic `lineage_version` + current `repoch` (D-82/D-87), and **the requester's per-zone live heads** — admission requires each named zone's live head to equal the bound one, so the assent dies the moment the lineage writes past it (a signed cutoff is never durable rollback authority, D-87) | reject-permanent |
| `c.abandon_writer` | admin | lineage abandoned at head | reject-permanent |
| `c.checkpoint` | admin | **zone-scoped (D-62), embedded (D-80), paged (D-88)**: the body IS the checkpoint object (App A.2 `checkpointobj`, no `v` — E6); the operation hash is its identity, the control log its carriage. `prev_checkpoint` chains the zone's checkpoints (`"genesis"` first); `covers` = a **page** of accepted heads (≤ 256; the zone's effective coverage = the union of the latest page per lineage — one operation never carries a full 4096-head Frontier); `fences` = per-lineage coordinates ≥ that lineage's covers head — **pending operations compare against `fences`, not accepted heads** (a pending op always sits beyond the accepted head, so v0.5.3's covers-relative rule could never fire — D-88); a fence coordinate asserts the author waited past it. `retired` = heads retired since the predecessor (≤ 256), each validated per D-76 (live at the predecessor, ≤ its lineage's fence, incorporation-eligible) — the fold removes **exactly `retired`**, never bare coverage; an omitted head simply stays live for a later page. `proof_positions` = currently qualified issuers only (≤ 64, `head_hash`-committed per D-87; closed feeds live behind their cutoffs). **Hardening (narrowed, D-77/D-88)**: a checkpoint whose lineage fence covers a pending operation's signed `(lineage, gen, seq)` AND whose committed proof feeds cover every qualified witness hardens `deadline-unreceipted`/`lease-missing` to quarantine; `causal-missing` hardens when the fence covers the missing predecessor's coordinates and it is absent; `cert-superseded`/`policy-missing` never fence-harden | reject-permanent |
...
admit_ctrl(op, control_state):
  ...
  place  arm admissibility (§7.2) + placement C1–C5; **C3′ validity
         is established here, BEFORE its precedence exception
         applies** (an invalid recovery op never suppresses C2)
  body   registry row; body CDDL; cross-field invariants (§7.1)
                                               → body outcomes
```
...
`bundle_size` and `content_digest` are **admission-time-validated
signed facts (D-90)**: checked where the sources are readable (the
authoring daemon at first admission), accepted as signed values by
every later replica — like any body invariant, never re-derived
(the charge and the digest thereby survive source erasure).
...
resolution; C3′ validity-before-precedence — D-91); checkpoint chain
(prev linkage, since-predecessor retired, embedded-object fold
transition, page-union coverage, fence-vs-pending coverage — D-88);
...
on every replica; `head_hash` boundary verification; repeated-cutoff
min-merge; device compromise cutoffs cover leases — D-87);
dropped-witness forever-pending lane (D-88)); 10 lineage/budget (generation
...
orders; signed window anchor; portable currency
...
matching; signed `bundle_size` accounting (mismatch rejects; admission-
validated fact replayed after erasure — D-90); frozen-stamp
PendingXfer recovery; erased-source aborts ALL remaining vs
destination-reject completes the rest (D-90); export_id
single-use;
...
later rotation's acceptance + Fence-closed intent (D-89); renewal
...
checkpointobj = { zone_id: ulid,   ; NO v — the envelope's
  ...
  retired: [* head],                 ; set (E7), key (lineage, gen);
                                     ;   <= 256; each live + eligible
                                     ;   (D-76) — the fold removes
                                     ;   exactly these, never bare
                                     ;   coverage
...
cenrollrenew = { cert: cert,                ; cert.renews REQUIRED
  history_cutoffs: [+ zonecutoff],
      ; set (E7), <= 64; accepted_through: "none" = the zero-history
      ;   sentinel; zones beyond 64 are covered by pre-established
      ;   standalone c.cutoff ops — validation requires the UNION to
      ;   cover every zone the device holds wraps for (D-89)
  ? wraps: [* kekwrap] }
      ; REQUIRED iff the renewed cert rotates its KEM key: one wrap
      ;   per accepted, unretired epoch of every held zone, each
      ;   superseding the predecessor's (zone, epoch, device) wrap;
      ;   ABSENT when the KEM key is unchanged (old wraps stand) —
      ;   D-89; grants and lineage NEVER appear (D-84)
...
cwrapadd = { zone_id: ulid, epoch: uint, wrap: kekwrap }
  ; epoch: any accepted, unretired epoch (>= the active epoch) —
  ;   D-89; same-(zone, epoch, device) wrap supersedes the prior
...
clineagereauth = { lineage: bytes16, max_generations: uint,
  ...
  ;   (D-82/D-87: lineage_version = the count of accepted
  ;   c.lineage_reauth ops for this lineage, 0 at creation, monotone,
  ;   zone-free; repoch equality defeats C3′ branch-cut resurrection
  ;   — recovery always bumps repoch)
...
zonecutoff = { zone_id: ulid, lineage: bytes16,
  ...
  ; multiple ACCEPTED cutoffs for one (zone_id, lineage) compose at
  ;   the MAXIMUM accepted_through — retirement is monotone (D-92);
...
ccutoff = { cutoffs: [+ zonecutoff],
  ...
  ;   lineage_version equality + repoch equality); live_heads = the
  ;   requester's current head per named zone ({zone_id,
  ;   head / "none"} set) — admission requires each to still be that
  ;   zone's live head, so assent invalidates the moment the lineage
  ;   writes past it (banked rollback authority dies)
