...
*v0.5.1 draft (freeze candidate), 2026-07-12. Supersedes v0.1–v0.5
...
- **E7 (sets and sort keys).** Every array documented as a *set* is
  ...
  `(zone_id, epoch, recipient_device)`; zone/history cutoffs →
  `(zone_id, lineage)`; `proof_cutoffs`/`receipt_cutoffs` → `key_id`;
  `revoke_grants` → grant id; `rotation_refs` → op hash;
...
- **E8 (hard caps — deterministic parsing).**
  ...
  | rotation `erase_manifest` (typed entries) | ≤ 192 (arithmetic: 256 wraps + 192 entries fit row 1; the erase queue drains across successive rotations, §5.4) |
...
`reauth`, `cutoffreq`, `survivors`. Derivation domains:
...
- Renewal: `c.enroll` with the same `device_id` + `renews`, carrying
  ...
  **superseded**: operations **already admitted at the pre-renewal
  frontier** and at or before the cutoffs remain valid (this keeps
...
`c.checkpoint` covers it — the frontier retains **only the active
...
- **T2 (witness qualification).** A receipt qualifies for an
  ...
  **service-key descriptor** (`c.service_key`, §7.1) — the descriptor,
  ...
  anchored at the citing operation (D-57, refining D-48)**: a receipt
  or lease qualifies for an operation iff the witness policy — and,
  for service issuers, the service-key descriptor — **in force at the
  citing operation's admission position in the fold** lists the
  issuer. Receipts carry no policy anchor of their own and need none:
  qualification is a pure function of (receipt bytes, citing-op
  position, control history), identical on every replica. Admitted
  operations are never re-litigated (folds replay deterministically
  at fixed positions), so a later `c.zone_policy` affects later
  admissions only; an operation still pending when policy changes is
  judged by the policy at its eventual admission position. **Lease
  issuers** are the same qualified witness set
...
- **T3 (anti-replay and issuer lifecycle).** `issuer_seq` is strictly
  ...
  `(service, key_id)` for service issuers — **one counter per scope,
  shared by receipts and leases alike, starting at 1** — persisted in
  the issuer's own durable store; **key renewal restarts the scope at 1** (a new
...
an idempotent skip. A queue larger than one manifest (> 192 entries,
                     ...
                     { kek_epoch, rotation_op: bytes32, count: uint,
                       ...
                       survivors: H_survivors( sorted array of
                         { item_addr, wrap_hash: SHA-256(canonical
                           ItemWrap bytes) } ) }
                     expected set = every item committed at or before
                     fence_frontier under the old epoch MINUS the
                     rotation's manifest item_addrs — completeness is
                     provable, not merely self-consistent
...
- **Cross-zone transfer (same daemon)** is two commits in two zone
  ...
  with no terminal record re-derives the bundle deterministically from
  the live sources (redaction is deterministic) and re-runs only the
  missing imports, then writes the terminal record. **Terminal states
  ...
  (nothing imports). **A PendingXfer clears only on its matching
...
| `c.service_key` | admin | installs/rotates a **service-key descriptor** `{ service: "connect", alg, pk, valid_from_admin_epoch, ? receipt_cutoff }` — the resolvable verification key behind a service issuer's `key_id` (D-48). `valid_from_admin_epoch` MUST equal the admin epoch at acceptance; a second descriptor for the same `(service, epoch)` rejects (`body-invariant`); a descriptor's validity runs from its acceptance to the next accepted descriptor for that service, selected at the citing operation's position (D-57). **Compromise remedy (D-58)**: a rotation MAY carry `receipt_cutoff` — the predecessor key's receipts with `issuer_seq` beyond it are retro-disqualified (T4 mechanics, mirroring device compromise); receipts within the qualified window stand — a stated residual, consistent with zone-policy-chooses-witnesses | reject-permanent |
| `c.grant` / `c.revoke_grant` | admin | issue / revoke capability | reject-permanent |
| `c.cap_epoch_bump` | admin | zone capability epoch +1 — body `new_epoch` MUST equal the zone's current capability epoch + 1 (`body-invariant`; §9.4 consecutiveness, consumed at validation); budget accounting resets | reject-permanent |
...
| `c.kek_rotate` | admin | §5.5 machine; wrap-adds; optional typed erase_manifest (≤ 192 entries, E8 — the erase queue drains across successive rotations, §5.4) | reject-permanent |
| `c.cutoff` | admin (hosted: own lineage only, §7.5) | zone-qualified cutoff `{ zone_id, lineage, accepted_through }`; MAY carry a **requester attestation** `Sign(device_sk, msg("cutoffreq", { plane_id, zone_id, lineage, accepted_through, request_id, ctrl_frontier }))` — REQUIRED on hosted planes (D-54), where it must prove the requesting device's own lineage; bound to this operation's `request_id`, single-use via D-59 | reject-permanent |
| `c.abandon_writer` | admin | lineage abandoned at head | reject-permanent |
| `c.checkpoint` | admin | **zone-scoped (D-62)**: names its `zone_id`; `covers` = that zone's Frontier hash; GC fence + **`proof_cutoffs`** (per qualified witness `(key_id, through issuer_seq)` — `key_id` = `H_key({alg, pk})` is collision-free by construction, so the scope is uniquely typed; qualification per that zone's policy at the fence's position, D-57): the fence commits receipt-feed positions, so pending-dependency **deadline and lease** operations harden deterministically — a fence covering the op's position **and** every qualified witness feed without the proof hardens it to quarantine; otherwise it stays pending (convergent on every replica) | reject-permanent |
...
- **(c)** admissible control operations: `c.enroll` (hosted-browser,
  ...
  wrap), `c.revoke_device` (compound), **`c.kek_rotate` only as a
  named part of an in-progress hosted exclusion**: an accepted
  `c.revoke_device` already names this rotation's op hash in its
  `rotation_refs` (hosted exclusion is compound-first — the compound
  waits, pending-dependency, on its rotations; D-50's rotation-first
  ordering stays a trusted-plane option) **and** the rotation's
  `erase_manifest` is empty — no standalone rotation under the
  ceiling (`c.revoke_zones` is unreachable here: a hosted plane
...
receipt (T2/T4, anchored per D-57) exists with `seen_ms ≤ D` — **a
...
chain:   fork, gen-first-op, lineage-gen, cutoff, capability-epoch,
         budget, duplicate, request-fork, ctrl-fork,
...
| `m.audit` | **audit.write** (the genesis audit grant, §7.1 — grantable to service actors only) | maudit | actor.kind == service; attested_by == the writing device's own cert; space == audit; works on **any device class** (a zero-daemon browser audits its own audited reads). **Trigger (D-64)**: a read/search is audited iff its scope includes a space with `class_minimum = sensitive` **or** its result set contains any item whose *effective* classification is `sensitive` (at genesis no space is sensitive-minimum, so the lane first fires with sensitive content — by design). **Body**: typed `principal` (the §10.1 shape plus its identifiers — never free text), `read_id` (minted per read), `chunk {index, count}`, `at_ms` = **diagnostic local time** (chronology, never authority). **Physical rule**: one read's audit rows ride **one `Txn`**; the edge caps result sets at 4096 IDs per request (16 records × 256 IDs, E8) so that Txn always fits; results release only after it is durable. **Failure/availability rules (D-52)**: an audited read whose audit Txn cannot durably commit **fails closed** with the dedicated outcome `audit-unavailable` — a read-only replica (lock loser, §6.2 L3) therefore cannot serve audited reads, and audit-budget exhaustion is a visible, owner-remediable denial, never a silent darkening (**hosted remedy = re-root**, D-64: no in-ceiling budget refresh exists; product copy warns near exhaustion); audit-space reads are themselves audited — writing an audit row is a write, so recursion terminates | audit rows appended (one Txn per read) | 1 op per row |
...
- Auto-context: status accepted ∧ live pin ∧ the pin's
  ...
  grants).
- Audit: audited reads/searches (trigger D-64, §11.1:
  sensitive-minimum space in scope, or any effective-sensitive item
  in the results) append `m.audit` rows (App A.5 `maudit` — typed
...
bundlerec    = { v: 1, op: bytes32, kind: kind, statement: text,
                 class_floor: class }     # deterministic redaction —
                                          ...
                                          # statement verbatim;
                                          # class_floor = the source's
                                          # effective classification
                                          # at the release's stamped
                                          # evaluation point
bundle       = { v: 1, export_id: bytes16,
                 recs: [* bundlerec] }    # recs: set, sorted by op —
                                          # EXACTLY the release's
                                          # sources (1:1, keyed set)
...
stamp shape, portable not replica-local). **Sources** are claims
...
witness policy at the import's admission position (D-57 — the flow is
source-governed). **Egress releases consume the deadline at release
acceptance**: the release operation itself requires the qualified
receipt by `D` (no import ever exists). Witnessless zones:
...
exclusion-bound rotation admitted vs standalone rejected,
recovery-succession admissibility, attested self-cutoff,
lineage_reauth self-service, consumed request_ids (`request-fork`),
drill acceptance; class table incl. mobile-attested vs mobile; renewal
...
unusability; `receipt_cutoff` retro-disqualification;
citing-position qualification across a policy change); 10 lineage/budget (generation
...
audit trigger both branches + typed principal + chunking + the 4096
edge cap; export_id single-use; XferAbort terminal (erased source);
...
recovery; zone-scoped frontier + checkpoint; cross-process lock;
erase crash matrix ×6 states incl. survivor-completeness failure
blocking KEK destruction;
...
| D-57 | Receipt/lease qualification (witness policy + service-descriptor selection) anchors at the **citing operation's admission position** — zero new receipt bytes; refines D-48; unifies T2/T4 | Ratified, owner, 2026-07-12 |
| D-58 | Service-key compromise remedy: `c.service_key` rotation with optional `receipt_cutoff` (predecessor receipts beyond it retro-disqualified); same-(service, epoch) duplicates reject; `valid_from_admin_epoch` = epoch at acceptance; pre-cutoff history stands (stated residual) | Ratified, owner, 2026-07-12 |
...
| D-64 | Audit exactness: trigger = sensitive-minimum space in scope ∨ effective-sensitive result; typed principal + `read_id` + chunk identity; one-Txn physical rule with the 4096-ID edge cap; `at_ms` diagnostic; hosted audit-budget remedy = re-root (no in-ceiling refresh) | Ratified, owner, 2026-07-12 |
...
| D-67 | Survivor completeness: `RewrapComplete` freezes `fence_frontier`; expected membership = old-epoch items at the fence minus the manifest; post-fence commits use the new epoch; old-KEK destruction gated on membership equality, not self-consistency | Ratified, owner, 2026-07-12 |
| D-68 | Mechanical batch: keyed-set logical identities (E7); chain arithmetic (seq from 1, +1; g = max + 1); genesis cross-field pins (provenance-compatible first cert class, grant epochs = 1, exact spaces/zone/lineage, wrap↔cert key equality, ordinary grant does NOT read audit space); `issued_admin_epoch` audit-only; zero-history `"none"` sentinel; enroll lineage absent-on-renewal; revocation exactness (complete grant set, rotation-after-last-wrap, any-order continuations); erase-manifest cap 192 with cross-rotation drain; `c.cap_epoch_bump` body validation; `draw_order` {name, nbytes}; drift sweep (zone-qualified recovery cutoffs, enroll/reauth split, drill custody wording, device service writer, T4 receipt_cutoffs, `c.checkpoint` naming, §11.5 evidence ref) | Ratified, owner, 2026-07-12 |
...
cservicekey = { service: "connect", alg: sigalg, pk: bstr,
  valid_from_admin_epoch: uint,   ; MUST equal epoch at acceptance
  ? receipt_cutoff: uint }        ; D-58: predecessor key's receipts
                                  ;   beyond this issuer_seq are
                                  ;   retro-disqualified
kekwrap = { v: 1, plane_id: bytes32, zone_id: ulid, epoch: uint,
...
cenroll = { cert: cert, grants: [* grant],   ; grants set (E7)
  ? lineage: lineagedef,   ; REQUIRED for a new device; ABSENT on
                           ;   renewal (D-68)
  wraps: [* kekwrap],                        ; set (E7)
  ? history_cutoffs: [+ zonecutoff] }
                           ; REQUIRED iff cert.renews present;
                           ;   accepted_through: "none" = the typed
                           ;   zero-history sentinel
...
clineagereauth = { lineage: bytes16, max_generations: uint,
  ...
  ;   bound to this op's request_id (consumed plane-wide, D-59)
...
zonecutoff = { zone_id: ulid, lineage: bytes16,
  accepted_through: head / "none" }   ; "none" = zero-history sentinel
ccutoff = { cutoff: zonecutoff,
  ? requester: { device_cert: bytes32, ctrl_frontier: bytes32,
                 sig: bstr } }
  ; requester REQUIRED on hosted planes (own lineage only — D-54);
  ;   sig = Sign(device_sk, msg("cutoffreq", { plane_id, zone_id,
  ;   lineage, accepted_through, request_id, ctrl_frontier })) —
  ;   single-use via consumed request_ids (D-59)
cabandon = { zone_id: ulid, lineage: bytes16, at: head }
ccheckpoint = { zone_id: ulid,        ; zone-scoped (D-62)
  checkpoint: bytes32, covers: bytes32, gc_fence: uint,
  proof_cutoffs: [* { key_id: bytes32, through: uint }] }
                                        ; set (E7), keyed by key_id
...
xferabort = { export_id: bytes16,
  ...
  missing: [* bytes32] }     ; set (E7): source_ops never imported —
                             ;   terminal (D-65)
...
auditprin = { shape: 1, device: bytes16 }
          ...
          / { shape: 4, peer: text, ? token_hash: bytes32 }
maudit = { principal: auditprin, read_id: bytes16,
  chunk: { index: uint, count: uint },
  scope: { zone: ulid, space: ulid },
  result_ids: [* bytes32],   ; set (E7)
